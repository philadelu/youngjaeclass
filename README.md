...
/* [수정] width 애니메이션 제거, transform만 사용 (성능 최적화) */
...
.problem-content { 
    ...
    align-items: center; 
    justify-content: start; /* center에서 start로 변경하여 상단부터 배치하되 패딩으로 밀어냄 */
    overflow-y: auto; 
    padding-top: 1rem; /* 상단 여백 확보 (헤더 겹침 방지) */
    padding-bottom: 2rem;
    padding-left: 1rem;
    padding-right: 1rem;
    word-break: keep-all; 
        ...
        .reading-content-wrapper strong { 
            color: #000; 
            font-weight: 700; 
            border-bottom: 1px solid #000; /* 형광색 대신 밑줄로 변경 */
            background: none;
            padding: 0;
        }
...
/* [중요] 색상 강제 적용 (!important 유지) */
.calendar-grid-container .sunday { color: #ef4444 !important; } /* 일요일 빨강 */
.calendar-grid-container .saturday { color: #2563eb !important; } /* 토요일 파랑 */
.calendar-grid-container .holiday { 
    color: #ef4444 !important; /* 공휴일 빨강 */
    background-color: #fef2f2; /* 배경 연한 빨강 */
}
...
.quiz-option-btn {
            ...
            border-radius: 0.5rem;
            color: #111;
            font-size: clamp(0.9rem, 1.2vw, 1.2rem); /* 폰트 사이즈 약간 축소 */
            font-weight: 700;
            padding: 0.25rem; /* 패딩 대폭 축소 */
            transition: all 0.1s;
    ...
    <div id="auth-modal" class="fixed inset-0 bg-gray-900/90 z-50 flex items-center justify-center backdrop-blur-sm fade-in hidden">
        <div class="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md border border-gray-200">
            <div class="text-center mb-8">
                ...
                <p class="text-[10px] text-purple-600 mt-1 font-bold">* 구글 스프레드시트 연동됨</p>
            </div>
            <div class="flex border-b border-gray-200 mb-6">
                <button onclick="switchAuthTab('login')" id="tab-login" class="flex-1 py-3 text-sm font-bold text-gray-900 border-b-2 border-gray-900 transition">로그인</button>
                <button onclick="switchAuthTab('signup')" id="tab-signup" class="flex-1 py-3 text-sm font-bold text-gray-400 hover:text-gray-600 transition">회원가입</button>
            </div>
            <div class="grid grid-cols-2 gap-3 mb-6">
                <button onclick="setRole('teacher')" id="btn-role-teacher" class="p-3 border-2 border-gray-200 rounded-xl flex flex-col items-center justify-center gap-1 transition text-gray-400 hover:border-gray-400">
                    <i class="fas fa-chalkboard-teacher text-xl"></i><span class="text-xs font-bold mt-1">선생님</span>
                </button>
                <button onclick="setRole('student')" id="btn-role-student" class="p-3 border-2 border-gray-200 rounded-xl flex flex-col items-center justify-center gap-1 transition text-gray-400 hover:border-gray-400">
                    <i class="fas fa-user-graduate text-xl"></i><span class="text-xs font-bold mt-1">학생</span>
                </button>
            </div>
            <div class="space-y-3">
                <input type="text" id="input-school" placeholder="학교 이름" class="auth-signup-field hidden w-full px-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:border-gray-800 focus:ring-0 outline-none text-sm transition font-medium">
                <input type="text" id="input-name" placeholder="이름" class="auth-signup-field hidden w-full px-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:border-gray-800 focus:ring-0 outline-none text-sm transition font-medium">
                <input type="text" id="input-id" placeholder="아이디" class="w-full px-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:border-gray-800 focus:ring-0 outline-none text-sm transition num-font font-medium">
                <input type="password" id="input-pw" placeholder="비밀번호" class="w-full px-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:border-gray-800 focus:ring-0 outline-none text-sm transition num-font font-medium">
            </div>
            <button onclick="processAuth()" id="btn-auth-submit" class="w-full bg-gray-900 hover:bg-black text-white font-bold py-4 rounded-xl transition shadow-xl mt-8 text-sm">로그인</button>
        </div>
    </div>
...
<aside id="main-sidebar" class="fixed inset-y-0 left-0 md:relative -translate-x-full bg-white w-64 border-r border-gray-200 flex flex-col z-50 h-full overflow-hidden flex-shrink-0 transition-transform duration-300">
        <div class="h-16 flex items-center px-8 border-b border-gray-100 flex-shrink-0">
            <h1 class="text-base font-black tracking-widest text-gray-900 cursor-pointer" onclick="goDashboard()">특수학급 학습</h1>
        </div>
...
<div class="flex items-stretch justify-between w-full gap-2 mt-2">
        <button id="btn-my-info" onclick="openMyInfo()" class="hidden flex-1 py-3 text-xs font-bold text-gray-600 bg-white border border-gray-200 hover:border-gray-400 hover:text-gray-900 rounded-xl transition flex items-center justify-center gap-2 shadow-sm">
            <i class="fas fa-chart-pie"></i> 내 성적표
        </button>
        ...
        <button id="btn-class-status" onclick="openTeacherStatus()" class="hidden flex-1 py-3 text-xs font-bold text-white bg-gray-900 hover:bg-black rounded-xl transition flex items-center justify-center gap-2 shadow-lg">
            <i class="fas fa-chalkboard-teacher"></i> 학급 현황
        </button>
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden bg-gray-50 relative">
        <header class="bg-white h-16 border-b border-gray-200 flex items-center justify-between px-6 flex-shrink-0 shadow-sm z-20">
            <div class="flex items-center gap-4">
                ...
                <h2 id="header-title" class="text-lg font-bold text-gray-800">대시보드</h2>
            </div>
...
<div id="view-dashboard" class="absolute inset-0 p-4 md:p-8 overflow-y-auto space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div class="bg-white p-6 rounded-2xl border border-gray-300 shadow-sm flex flex-col justify-center">
                        <p class="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 flex items-center gap-1"><i class="fas fa-school"></i> 나의 학교</p>
                    ...
                    <div class="bg-white p-6 rounded-2xl border border-gray-300 shadow-sm flex flex-col relative h-40">
                        <div class="flex justify-between items-center mb-2">
                            <p class="text-[12px] font-bold text-gray-500 uppercase tracking-widest flex items-center gap-1"><i class="fas fa-bullhorn"></i> 공지사항</p>
                            <span id="memo-status" class="text-[12px] text-gray-900 font-bold hidden bg-gray-200 px-2 py-0.5 rounded">SAVED</span>
                        </div>
                        <textarea id="dash-memo" class="flex-1 w-full bg-transparent resize-none outline-none text-sm text-gray-800 leading-relaxed font-medium placeholder-gray-300" placeholder="공지사항을 입력하세요."></textarea>
                    </div>
                </div>
                <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden p-6">
                    <div class="flex justify-between items-center mb-4">
                        <div>
                            <h3 class="font-black text-gray-900 text-xl tracking-tight">순위</h3>
                        </div>
                        <span class="text-xs font-bold text-gray-400 bg-gray-100 px-2 py-1 rounded">실시간</span>
                    </div>
                    <div id="ranking-list" class="space-y-3">
                        <div class="p-8 text-center text-gray-400 text-sm">데이터를 불러오는 중...</div>
                    </div>
                </div>
            </div>
            ...
            <div id="view-student" class="absolute inset-0 bg-gray-50 hidden z-20 flex flex-col items-center justify-center p-6">
                <div id="quiz-ready-screen" class="absolute inset-0 z-30 bg-white/95 flex flex-col items-center justify-center p-8 text-center backdrop-blur-sm">
                    <h2 class="text-4xl font-black text-gray-900 mb-2">준비되었나요?</h2>
                    ...
                    <div class="bg-gray-100 p-6 rounded-2xl mb-8 max-w-sm w-full border border-gray-200">
                        <p class="text-sm text-gray-500 font-bold uppercase mb-2">목표점수</p>
                        <p class="text-5xl font-black text-gray-900 num-font mb-4">50<span class="text-2xl text-gray-400 ml-1">점</span></p>
                        <div class="flex justify-between text-sm font-medium text-gray-500 border-t border-gray-200 pt-2">
                        </div>
                    </div>
                    <button onclick="startQuizFlow()" class="bg-gray-900 hover:bg-black text-white text-xl font-bold py-4 px-12 rounded-full shadow-xl transition transform hover:scale-105">도전하기</button>
                    <button onclick="goDashboard()" class="mt-6 text-gray-400 hover:text-gray-600 text-sm underline">나가기</button>
                </div>
                ...
                <div class="w-full max-w-4xl bg-white flex flex-col h-full rounded-3xl shadow-2xl border border-gray-200 overflow-hidden relative">
                    <div class="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-8 shrink-0">
                        ...
                        <div class="flex items-center gap-2">
                            <span class="text-xs font-bold text-gray-400 uppercase">점수</span>
                        ...
                        <button onclick="finishQuiz()" class="text-gray-400 hover:text-gray-900 font-bold text-sm underline shrink-0 ml-4">그만하기</button>
                    </div>
                  ...
                  <div id="quiz-options" class="h-auto bg-white border-t border-gray-200 p-2 grid grid-cols-2 gap-2 shrink-0"></div>
                    ...
                    <div id="quiz-result-view" class="absolute inset-0 bg-white z-40 hidden flex-col items-center justify-center p-8 text-center animate-fade-in">
                        ...
                        <button onclick="goDashboard()" class="bg-gray-900 hover:bg-black text-white text-lg font-bold py-4 px-10 rounded-xl shadow-lg transition w-full max-w-sm">대시보드로 돌아가기</button>
                    </div>
                </div>
            </div>
            ...
            <div id="view-teacher" class="absolute inset-0 bg-gray-900 hidden flex flex-col z-20">
                <div class="h-16 bg-gray-900 border-b border-gray-800 flex items-center justify-between px-6 shrink-0">
                    <div class="flex items-center gap-4"><span id="board-title" class="text-white font-bold text-lg tracking-tight">주제</span></div>
 <div class="flex items-center gap-3">
                        <button onclick="generateBoardProblem()" class="bg-gray-800 hover:bg-gray-700 text-white px-4 py-2 rounded-lg text-sm font-bold border border-gray-700">문제 띄우기</button>
                        ...
                        <button id="btn-board-mode" onclick="toggleBoardMode()" class="bg-green-600 hover:bg-green-500 text-white px-3 py-2 rounded-lg text-sm font-bold border border-green-700 shadow transition flex items-center gap-2">
                            <i class="fas fa-pen"></i> <span>필기모드</span>
                        </button>
                        ...
                        <button onclick="goDashboard()" class="text-gray-400 hover:text-white font-bold text-sm ml-2 bg-gray-800 px-3 py-1.5 rounded-lg border border-gray-700">나가기</button>
                    </div>
                </div>
            ...
            <div id="view-learning" class="hidden absolute inset-0 bg-[#18181b] z-[60] flex-col text-white">
                <div class="h-16 flex items-center justify-between px-6 border-b border-gray-700 bg-gray-900/50">
                    <h2 class="text-lg md:text-xl font-bold text-gray-400">
                        수업 모드 <span class="mx-2 text-gray-600">|</span> <span id="learning-subject-title" class="text-white text-xl md:text-2xl font-black"></span>
                    </h2>
                    <button onclick="exitLearningMode()" class="bg-red-600 hover:bg-red-500 text-white font-bold py-2 px-6 rounded-lg transition">나가기</button>
                </div>
                ...
                <div class="flex-1 flex overflow-hidden relative">
                    <div class="flex-1 relative border-r-2 border-gray-700 bg-[#202025] flex flex-col">
                        <div id="learning-problem-area" class="w-full h-full overflow-y-auto flex flex-col items-center justify-start p-10 z-[1]">
                            <span class="text-gray-500">주제를 선택하고 시작해주세요.</span>
                        </div>
                    ...
                    <div id="learning-toolbar" class="absolute top-1/2 right-5 -translate-y-1/2 bg-gray-900/90 p-4 rounded-2xl flex flex-col items-center gap-3 z-[30] border border-gray-700 shadow-2xl backdrop-blur-sm transition-transform duration-300">
                        <div id="toolbar-toggle-btn" onclick="toggleToolbar()" class="absolute -left-7 top-1/2 -translate-y-1/2 w-7 h-16 bg-gray-900/90 border-y border-l border-gray-700 rounded-l-lg flex items-center justify-center text-yellow-400 cursor-pointer shadow-lg">▶</div>
                        
                        <button onclick="nextLearningProblem()" class="bg-blue-600 hover:bg-blue-500 text-white font-bold py-2 px-3 rounded-lg text-sm w-full">다음 ▶</button>
                        <div class="w-full h-px bg-gray-600 my-1"></div>
                        
                        <button id="btn-scroll-toggle" onclick="toggleScrollMode()" class="bg-gray-700 hover:bg-gray-600 text-white font-bold py-2 px-2 rounded-lg text-xs w-full">👆 스크롤</button>
                        <div class="w-full h-px bg-gray-600 my-1"></div>
                        ...
                        <div class="w-7 h-7 rounded-full bg-gray-600 border-2 border-gray-400 cursor-pointer hover:scale-110 transition flex items-center justify-center text-xs" onclick="setLearnEraser(this)">🧹</div>
                        
                        <div class="w-full h-px bg-gray-600 my-1"></div>
                        <button onclick="clearCanvas('left')" class="text-xs text-gray-400 hover:text-white">문제지움</button>
                        <button onclick="clearCanvas('right')" class="text-xs text-gray-400 hover:text-white">노트지움</button>
                    </div>
                </div>
            </div>
            ...
            <div id="view-my-info" class="absolute inset-0 bg-gray-900/50 hidden z-[60] flex items-center justify-center backdrop-blur-sm p-4">
                <div class="bg-white w-full max-w-4xl h-[90%] rounded-3xl shadow-2xl flex flex-col overflow-hidden">
                    <div class="px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-white">
                        <div>
                            <h2 class="text-2xl font-black text-gray-900 tracking-tight">학습 리포트</h2>
...
<thead class="bg-gray-50 text-xs font-bold text-gray-400 uppercase border-b border-gray-200">
    <tr>
        <th class="px-3 md:px-6 py-3 md:py-4 whitespace-nowrap">주제</th>
        <th class="px-3 md:px-6 py-3 md:py-4 whitespace-nowrap text-center">등급</th>
        <th class="px-3 md:px-6 py-3 md:py-4 whitespace-nowrap text-right">시간</th>
        <th class="px-3 md:px-6 py-3 md:py-4 whitespace-nowrap text-center">점수</th>
    </tr>
</thead>
            ...
            <div id="view-teacher-status" class="absolute inset-0 bg-gray-900/50 hidden z-50 flex items-center justify-center backdrop-blur-sm p-4">
                <div class="bg-white w-full max-w-5xl h-[90%] rounded-3xl shadow-2xl flex flex-col overflow-hidden border border-gray-200">
                    <div class="px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                        <div>
                            <h2 class="text-2xl font-black text-gray-900 tracking-tight">📊 학급 학습 현황</h2>
                            <p class="text-xs text-gray-500 mt-1">이름을 클릭하면 상세 성적표를 볼 수 있습니다.</p>
                        </div>
    ...
    <div id="data-modal" class="fixed inset-0 bg-black/80 z-[100] hidden flex items-center justify-center p-4 backdrop-blur-sm">
        <div class="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl animate-fade-in">
            <div class="p-4 border-b flex justify-between items-center bg-gray-50">
                <h3 id="modal-title" class="text-xl font-black text-gray-800">자료 보기</h3>
            ...
            <div class="p-4 border-t bg-gray-50 text-center">
                <button onclick="closeModal()" class="py-4 px-12 bg-gray-900 text-white text-xl font-bold rounded-2xl hover:bg-black transition shadow-lg">닫기 (문제 풀기)</button>
            </div>
        </div>
    </div>
    
 <script>
   let currentQuiz = null; // 현재 풀고 있는 문제 데이터 저장
let sidebarOpen = false; // 사이드바 상태
        // -----------------------------------------------------------
        // [Step 3] 기본 설정 및 데이터 뱅크 (Code 2 데이터 완벽 이식)
        // -----------------------------------------------------------
        
        // [Google Apps Script URL] (기존 1번 코드 유지)
...
let AppState = { 
        ...
        var currentModalContent = ""; // 모달에 띄울 내용 저장

        // ==========================================
        // [1] 필수 헬퍼 함수 (Code 1 + Code 2 통합)
        // ==========================================
        
        // 한글 자모 데이터
        const H_CHO = ['ㄱ', 'ㄲ', 'ㄴ', 'ㄷ', 'ㄸ', 'ㄹ', 'ㅁ', 'ㅂ', 'ㅃ', 'ㅅ', 'ㅆ', 'ㅇ', 'ㅈ', 'ㅉ', 'ㅊ', 'ㅋ', 'ㅌ', 'ㅍ', 'ㅎ'];
        const H_JUNG = ['ㅏ', 'ㅐ', 'ㅑ', 'ㅒ', 'ㅓ', 'ㅔ', 'ㅕ', 'ㅖ', 'ㅗ', 'ㅘ', 'ㅙ', 'ㅚ', 'ㅛ', 'ㅜ', 'ㅝ', 'ㅞ', 'ㅟ', 'ㅠ', 'ㅡ', 'ㅢ', 'ㅣ'];
        const H_JONG = ['', 'ㄱ', 'ㄲ', 'ㄳ', 'ㄴ', 'ㄵ', 'ㄶ', 'ㄷ', 'ㄹ', 'ㄺ', 'ㄻ', 'ㄼ', 'ㄽ', 'ㄾ', 'ㄿ', 'ㅀ', 'ㅁ', 'ㅂ', 'ㅄ', 'ㅅ', 'ㅆ', 'ㅇ', 'ㅈ', 'ㅊ', 'ㅋ', 'ㅌ', 'ㅍ', 'ㅎ'];
   
function toggleSidebar() {
    sidebarOpen = !sidebarOpen;
    const sb = document.getElementById('main-sidebar');
    const overlay = document.getElementById('sidebar-overlay'); // 오버레이 요소 가져오기

    if (sidebarOpen) { 
        sb.classList.remove('-translate-x-full');
        if(overlay) overlay.classList.remove('hidden'); // 오버레이 보이기
    } else { 
        sb.classList.add('-translate-x-full');
        if(overlay) overlay.classList.add('hidden'); // 오버레이 숨기기
    }
}
    ...
    function switchAuthTab(mode) {
        ...
        document.getElementById('btn-auth-submit').innerText = mode==='login' ? '로그인' : '회원가입';
    ...
    function saveRecordToServer(record) {
        AppState.records.push(record); // 로컬 선반영
        const params = new URLSearchParams({ action: 'saveRecord', data: JSON.stringify(record) });
        fetch(GAS_URL, { method: 'POST', body: params }).catch(e => console.log("기록 저장 실패(백그라운드)"));
    }
  ...
  function processAuth() {
        ...
        if(authMode === 'login') {
            ...
            if(!id || !pw || !name || !selectedRole) { alert("모든 정보를 입력해주세요."); return; }
            
            // [수정됨] 중복 체크 시에도 String 변환
            if(AppState.users.find(u => String(u.id) === id)) { alert("이미 존재하는 아이디입니다."); return; }
            ...
            fetch(GAS_URL, { method: 'POST', body: params })
                .then(r => r.text())
                .then(t => { 
                    if(t!=='success') console.error("서버 저장 실패: " + t); 
                })
...
function loginSuccess(user) {
 ...
 document.getElementById('profile-name').innerText = user.name + (user.role==='teacher'?' 선생님':' 학생');
    document.getElementById('profile-school').innerText = user.school;
    
    // [수정] 대시보드 메인 화면에도 학교 이름 반영
  ...
  if (user.role === 'student') {
        ...
        memoEl.value = AppState.memo || ""; // 데이터 없으면 빈 문자열
        ...
        renderSidebarMenu();
        // [수정] 로그인 후 즉시 대시보드 화면으로 이동하도록 호출
        goDashboard();
    }
function handleLogout() {
    if(confirm("로그아웃 하시겠습니까?")) {
        ...
        // 2. 화면 강제 전환 (새로고침 없이 즉시 로그인 화면으로)
        ...
        // 4. 안전을 위해 리로드 (선택사항이나 깔끔한 초기화를 위해 권장)
        setTimeout(() => location.reload(), 100);
    }
}

    // 4. 메뉴 및 랭킹 렌더링
// 4. 메뉴 및 랭킹 렌더링 (아코디언 스타일로 변경)
    function renderSidebarMenu() {
        const list = document.getElementById('sidebar-menu');
        list.innerHTML = "";
    MenuData.forEach((cat, idx) => {
            // 모든 메뉴 접힌 상태로 시작 (open 속성 제거)
            const isOpen = ''; 
            let html = `
 ...
 function updateRankingBoard() {
        ...
        AppState.records.forEach(r => {
            ...
            // 해당 과목이 없거나, 더 높은 점수(또는 같은 점수면 더 빠른 시간)일 때 갱신
        ...
        if (sorted.length === 0) {
            list.innerHTML = `<div class="p-4 text-center text-gray-400">아직 랭킹 데이터가 없습니다.</div>`;
            return;
        }

 sorted.forEach((item, idx) => {
            // [수정] 이모티콘 제거, 심플한 숫자 배지 및 흑백 톤 디자인 적용
            let badgeClass = "bg-gray-100 text-gray-400";
            if(idx === 0) badgeClass = "bg-black text-white shadow-md";       // 1등: 검정
            else if(idx === 1) badgeClass = "bg-gray-600 text-white"; // 2등: 진한 회색
            else if(idx === 2) badgeClass = "bg-gray-400 text-white"; // 3등: 회색
    ...
    function getNativeNumber(n) {
        const natives = ["영","하나","둘","셋","넷","다섯","여섯","일곱","여덟","아홉","열"];
        if(n <= 10) return natives[n];
        if(n < 20) return "열" + natives[n-10];
...
function getRank(seconds, targetScore) {
        // [수정] 만점(목표 점수) 미달 시 등급 없음 처리
        if (AppState.currentScore < targetScore) return "-";

        // 1문제당 평균 소요 시간 (또는 1점당, 여기서는 설정상 10점이 만점이므로 '시간 / 10'이 됨)
        // 설정값 limits가 '1.2초'라는 것은 1점을 얻는데 1.2초(즉 2점짜리 한 문제에 2.4초)를 의미하는지,
        // 아니면 단순히 '문제 풀이당 속도' 지표인지에 따라 다르지만, 
        // 보내주신 코드 로직(seconds / targetScore)을 그대로 따릅니다.
        const secPerPoint = seconds / targetScore;

        // 기본값 (설정이 없을 경우 대비)
        let baseS = 1.5, baseA = 2.5, baseB = 4.0;

        // 현재 주제의 설정(limits) 가져오기
...
function getCalendarHTML(md) {
    ...
    // 전체 셀 개수를 세는 변수 (헤더 제외하고 본문 칸 수만)
    let totalCount = 0;
    ...
    for(let d=1; d<=lastDate; d++) {
        let cls = ''; 
        const dayIdx = (startDayIndex + d - 1) % 7; // 0:일요일 ~ 6:토요일
    ...
    while (totalCount % 7 !== 0) {
        // [수정] style 속성으로 배경색 강제 지정 (검은색으로 보이는 문제 해결)
 ...
 function drawClock(id, h, m) {
  ...
  const width = cvs.width;
        ...
        const R = width / 2; 
        // [수정] 캔버스 꽉 채우지 않고 여백(Margin) 확보 (0.9배)
        const radius = R * 0.9; 
...
function refreshData() { 
        ...
        function getRandDate() { return `${getRandomInt(1, 12)}월 ${getRandomInt(1, 30)}일`; }
        ...
        // 보기(Distractors) 생성 헬퍼 (Code 2 로직 적용)
        function getDistractors(answer, type, sourceArr = []) {
            ...
            while(set.size < 4) {
                ...
                else set.add("기타" + set.size);
            }
            return shuffleArray(Array.from(set));
        }

        // ==========================================
        // [2] 국어/영어/상식 데이터 뱅크 (Code 2 전체)
        // ==========================================

        const bank_consonants = [
            // ㄱ
            {q:"학교 갈 때 등에 매는 것", a:"가방"}, {q:"멍멍! 집을 지키는 동물", a:"강아지"}, {q:"칙칙폭폭 길게 달리는 차", a:"기차"}, {q:"불에 구워 먹으면 맛있는 음식", a:"고기"}, {q:"다리가 8개, 거미줄을 쳐요", a:"거미"},
            {q:"하늘에 떠 있는 하얀 솜사탕", a:"구름"}, {q:"또각또각 소리 나는 신발", a:"구두"}, {q:"코끼리의 코는 아주 ⬜어", a:"길어"}, {q:"겨울에 먹는 달콤한 뿌리 간식", a:"고구마"}, {q:"무대에서 노래 부르는 사람", a:"가수"},
            
            // ㄴ
            {q:"꽃을 찾아 날아다니는 곤충", a:"나비"}, {q:"숲에 많은 초록색 식물", a:"나무"}, {q:"여자인 형제", a:"누나"}, {q:"마이크 잡고 부르는 것", a:"노래"}, {q:"강에서 물고기를 잡는 일", a:"낚시"},
            {q:"집 밖으로 ⬜오다", a:"나오다"}, {q:"논과 밭에서 짓는 것", a:"농사"}, {q:"해가 떠 있는 밝은 시간", a:"낮"}, {q:"봄에 도시락 싸서 가는 것", a:"나들이"}, {q:"점심 먹고 자는 잠", a:"낮잠"},

            // ㄷ
            {q:"도토리를 좋아하는 동물", a:"다람쥐"}, {q:"콩으로 만든 네모난 반찬", a:"두부"}, {q:"옷의 주름을 펴는 기계", a:"다리미"}, {q:"강을 건너게 해주는 길", a:"다리"}, {q:"쌉쌀한 맛이 나는 뿌리 반찬", a:"도라지"},
            {q:"별 모양의 바다 동물", a:"불가사리"}, {q:"망치, 톱 같은 일하는 물건", a:"도구"}, {q:"가운데 구멍 뚫린 빵", a:"도너츠"}, {q:"꿀꿀! 뚱뚱한 동물", a:"돼지"}, {q:"칼질할 때 쓰는 받침대", a:"도마"},

            // ㄹ
            {q:"소리를 듣는 기계", a:"라디오"}, {q:"꼬불꼬불 국물 요리", a:"라면"}, {q:"갈기 있는 동물의 왕 (영어)", a:"라이온"}, {q:"삐리삐리 기계 인간", a:"로봇"}, {q:"아주 신 노란 과일", a:"레몬"}, 
            {q:"입술에 바르는 화장품", a:"립스틱"}, {q:"놀이공원의 무서운 열차", a:"롤러코스터"}, {q:"우주로 날아가는 것", a:"로켓"}, {q:"선물 상자에 묶는 끈", a:"리본"}, {q:"계단 대신 타는 기계", a:"에스컬레이터"},

            // ㅁ
            {q:"감기 걸리면 입에 쓰는 것", a:"마스크"}, {q:"수리수리 얍! 신기한 쇼", a:"마술"}, {q:"아침마다 마시는 신선한 우유", a:"매일우유"}, {q:"머리에 쓰는 것", a:"모자"}, {q:"여름에 피를 빠는 곤충", a:"모기"},
            {q:"놀이터에서 타고 내려오는 것", a:"미끄럼틀"}, {q:"비 온 뒤 뜨는 일곱 색깔", a:"무지개"}, {q:"깍두기를 만드는 하얀 채소", a:"무"}, {q:"여름에 맴맴 우는 곤충", a:"매미"}, {q:"물건을 파는 큰 가게", a:"마트"},

            // ㅂ
            {q:"길고 노란 원숭이 간식", a:"바나나"}, {q:"많은 사람이 타는 큰 차", a:"버스"}, {q:"집에서 키우는 얼룩 강아지", a:"바둑이"}, {q:"비 올 때 입는 옷", a:"비옷"}, {q:"물범과 비슷한 바다 동물", a:"바다표범"},
            {q:"하늘을 나는 교통수단", a:"비행기"}, {q:"포도, 가지의 색깔", a:"보라색"}, {q:"한국인의 주식, 하얀 곡식", a:"밥"}, {q:"선풍기에서 나오는 것", a:"바람"}, {q:"거품을 내서 씻는 것", a:"비누"},

            // ㅅ
            {q:"어흥! 갈기가 멋진 동물", a:"사자"}, {q:"백설공주가 먹은 과일", a:"사과"}, {q:"등이 딱딱하고 느린 동물", a:"거북이"}, {q:"뿔이 멋진 숲속 동물", a:"사슴"}, {q:"초록 줄무늬, 속은 빨간 과일", a:"수박"}, 
            {q:"세수하고 손 씻는 곳", a:"세면대"}, {q:"고민을 들어주는 곳", a:"상담실"}, {q:"시간을 알려주는 물건", a:"시계"}, {q:"하트 모양 마음", a:"사랑"}, {q:"불을 끄러 출동하는 차", a:"소방차"}, 

            // ㅇ
            {q:"응애응애 태어난 사람", a:"아기"}, {q:"차갑고 달콤한 간식", a:"아이스크림"}, {q:"나를 낳아주신 분", a:"엄마"}, {q:"병원에서 급할 때 가는 곳", a:"응급실"}, {q:"해가 뜨고 학교 가는 시간", a:"아침"},
            {q:"길쭉하고 초록색인 채소", a:"오이"}, {q:"젖소에게서 나오는 하얀 물", a:"우유"}, {q:"눈 위에 있는 얼굴 부분", a:"이마"}, {q:"눈 나쁠 때 쓰는 것", a:"안경"}, {q:"비 올 때 쓰는 것", a:"우산"},

            // ㅈ
            {q:"붕붕 도로를 달리는 것", a:"자동차"}, {q:"땅 밑으로 다니는 기차", a:"지하철"}, {q:"우리가 가족과 사는 곳", a:"집"}, {q:"연필 글씨를 지우는 것", a:"지우개"}, {q:"따끔! 병원에서 맞아요", a:"주사"},
            {q:"줄을 넘으며 뛰는 운동", a:"줄넘기"}, {q:"바퀴 두 개로 가는 것", a:"자전거"}, {q:"접거나 자르며 노는 것", a:"종이"}, {q:"옷에 달린 물건 넣는 곳", a:"주머니"}, {q:"돈을 넣어 다니는 물건", a:"지갑"},

            // ㅊ
            {q:"칙칙폭폭 레일 위를 달려요", a:"기차"}, {q:"꽁꽁 언 물", a:"얼음"}, {q:"글과 그림이 있는 것", a:"책"}, {q:"이를 닦는 솔", a:"칫솔"}, {q:"여자들이 주로 입는 옷", a:"치마"},
            {q:"횡단보도의 안전한 불빛", a:"초록색"}, {q:"사이좋게 지내는 또래", a:"친구"}, {q:"먼지를 빨아들이는 기계", a:"청소기"}, {q:"생일 케이크 위에 꽂는 것", a:"촛불"}, {q:"벽에 있는 유리 문", a:"창문"},

            // ㅋ
            {q:"찰칵! 사진 찍는 기계", a:"카메라"}, {q:"주머니에 아기를 넣고 뛰는 동물", a:"캥거루"}, {q:"코가 아주 긴 동물", a:"코끼리"}, {q:"몸의 높이", a:"키"}, {q:"생일에 먹는 빵", a:"케이크"}, 
            {q:"산타 할아버지가 오는 날", a:"크리스마스"}, {q:"자판기와 마우스가 있어요", a:"컴퓨터"}, {q:"달콤하고 따뜻한 초코 음료", a:"코코아"}, {q:"머리가 노란 콩나물", a:"콩나물"}, {q:"문 여는 열쇠(영어)", a:"키"},

            // ㅌ
            {q:"알이 아주 크고 빠른 새", a:"타조"}, {q:"짐을 싣는 큰 자동차", a:"트럭"}, {q:"만화도 보고 뉴스도 봐요", a:"텔레비전"}, {q:"케첩을 만드는 빨간 채소", a:"토마토"}, {q:"귀가 길고 깡충 뛰는 동물", a:"토끼"},
            {q:"우리나라 국기", a:"태극기"}, {q:"돈을 내고 타는 승용차", a:"택시"}, {q:"기름에 바삭하게 요리한 것", a:"튀김"}, {q:"산을 뚫어 만든 길", a:"터널"}, {q:"크리스마스 나무", a:"트리"},

            // ㅍ
            {q:"라면에 넣는 초록 채소", a:"파"}, {q:"건반을 누르면 소리가 나요", a:"피아노"}, {q:"맑은 하늘의 색깔", a:"파란색"}, {q:"남극에 사는 신사 새", a:"펭귄"}, {q:"보라색 알맹이가 주렁주렁", a:"포도"},
            {q:"치즈가 쭉 늘어나는 빵", a:"피자"}, {q:"마음을 전하는 종이", a:"편지"}, {q:"윙윙 벌레를 잡는 채", a:"파리채"}, {q:"후~ 불면 커지는 것", a:"풍선"}, {q:"소가 뜯어 먹는 초록 식물", a:"풀"},

            // ㅎ
            {q:"입이 아주 큰 물가 동물", a:"하마"}, {q:"즐거울 때 나는 소리", a:"하하하"}, {q:"공부하러 가는 곳", a:"학교"}, {q:"구름이 떠 있는 곳", a:"하늘"}, {q:"떡 하나 주면 안 잡아먹지", a:"호랑이"},
            {q:"코 풀 때 쓰는 종이", a:"휴지"}, {q:"해만 바라보는 꽃", a:"해바라기"}, {q:"빵 사이에 고기 패티", a:"햄버거"}, {q:"할로윈의 주황색 채소", a:"호박"}, {q:"눈의 색깔", a:"하얀색"}
        ];

        const bank_vowels = [
            // ㅏ
            {q:"나를 사랑해주는 남자 어른", a:"아빠"}, {q:"기저귀를 차는 어린 사람", a:"아기"}, {q:"갈기가 멋진 동물의 왕", a:"사자"}, {q:"원숭이가 좋아하는 노란 과일", a:"바나나"}, {q:"물이 많고 파도 치는 곳", a:"바다"},
            {q:"다리에 입는 옷", a:"바지"}, {q:"입이 큰 동물", a:"하마"}, {q:"철길 위를 달리는 차", a:"기차"}, {q:"신호등의 건너가도 되는 색", a:"파랑"}, {q:"백설공주가 먹은 과일", a:"사과"},

            // ㅑ
            {q:"방망이로 공을 치는 운동", a:"야구"}, {q:"몸에 좋은 풀(채소)", a:"야채"}, {q:"고양이 울음 소리", a:"야옹"}, {q:"옛날 옛날에~ (스토리)", a:"이야기"}, {q:"털이 긴 소처럼 생긴 동물", a:"야크"},
            {q:"더운 나라에 있는 나무", a:"야자수"}, {q:"밤에 시켜 먹는 음식", a:"야식"}, {q:"산에서 자라는 동물", a:"야생"}, {q:"산 정상에서 외치는 소리", a:"야호"}, {q:"야구 경기를 하는 곳", a:"야구장"},

            // ㅓ
            {q:"나를 낳아주신 여자 분", a:"어머니"}, {q:"아이가 자라서 된 사람", a:"어른"}, {q:"금붕어가 사는 집", a:"어항"}, {q:"팔과 목 사이의 신체", a:"어깨"}, {q:"초등학교 다니는 아이", a:"어린이"},
            {q:"장소를 물을 때 하는 말", a:"어디"}, {q:"빨리 오라고 할 때", a:"어서"}, {q:"불이 꺼지면 찾아와요", a:"어둠"}, {q:"물고기 잡는 사람", a:"어부"}, {q:"생선살로 만든 쫄깃한 음식", a:"어묵"},

            // ㅕ
            {q:"꾀가 많은 동물", a:"여우"}, {q:"엄마, 누나, 이모의 성별", a:"여자"}, {q:"덥고 수영하는 계절", a:"여름"}, {q:"이 장소를 가리키는 말", a:"여기"}, {q:"가방 싸서 멀리 놀러가는 것", a:"여행"},
            {q:"전화 받을 때 하는 말", a:"여보세요"}, {q:"숫자 6을 읽는 말", a:"여섯"}, {q:"숫자 8을 읽는 말", a:"여덟"}, {q:"소가 먹는 밥", a:"여물"}, {q:"왕의 부인", a:"여왕"},

            // ㅗ
            {q:"길쭉하고 아삭한 초록 채소", a:"오이"}, {q:"꽥꽥 우는 새", a:"오리"}, {q:"다리가 10개인 바다 동물", a:"오징어"}, {q:"어제, 내일, 그리고 지금", a:"오늘"}, {q:"여동생의 남자 형제", a:"오빠"},
            {q:"점심 먹고 난 뒤의 시간", a:"오후"}, {q:"주황색 껍질 과일", a:"오렌지"}, {q:"머리에 쓰는 것", a:"모자"}, {q:"보라색 송이가 주렁주렁", a:"포도"}, {q:"차가 다니는 길", a:"도로"},

            // ㅛ
            {q:"음식을 만드는 일", a:"요리"}, {q:"마시는 새콤달콤 음료", a:"요구르트"}, {q:"줄을 감았다 푸는 장난감", a:"요요"}, {q:"월, 화, 수, 목...", a:"요일"}, {q:"동화 속 작고 신기한 존재", a:"요정"},
            {q:"마법사가 부리는 것", a:"요술"}, {q:"최근을 부르는 말", a:"요즘"}, {q:"몸을 유연하게 하는 운동", a:"요가"}, {q:"버스 탈 때 내는 돈", a:"요금"}, {q:"흔들흔들 아기 침대", a:"요람"},

            // ㅜ
            {q:"젖소가 주는 하얀 물", a:"우유"}, {q:"비 올 때 펴는 것", a:"우산"}, {q:"나와 너를 합친 말", a:"우리"}, {q:"지구 밖 별들이 있는 곳", a:"우주"}, {q:"오동통한 면 요리", a:"우동"},
            {q:"편지를 부치는 곳", a:"우체국"}, {q:"하늘에서 떨어지는 얼음 알갱이", a:"우박"}, {q:"친구와의 사랑", a:"우정"}, {q:"경기에서 1등 하는 것", a:"우승"}, {q:"목소리가 크고 씩씩하다", a:"우렁차다"},

            // ㅠ
            {q:"투명하고 깨지기 쉬운 것", a:"유리"}, {q:"학교 가기 전에 다니는 곳", a:"유치원"}, {q:"하얗고 무서운 귀신", a:"유령"}, {q:"노란 유자로 만든 차", a:"유자차"}, {q:"이름이 널리 알려진 것", a:"유명하다"},
            {q:"띠를 매고 하는 운동", a:"유도"}, {q:"박물관에 있는 옛날 물건", a:"유물"}, {q:"요플레에 들어있는 균", a:"유산균"}, {q:"뿔 달린 환상의 말", a:"유니콘"}, {q:"기름을 실어나르는 배", a:"유조선"},

            // ㅡ
            {q:"아이가 자라면?", a:"어른"}, {q:"최고라는 뜻", a:"으뜸"}, {q:"힘을 쓸 때 내는 소리", a:"으라차차"}, {q:"악기로 연주하는 것", a:"음악"}, {q:"밥, 반찬, 간식", a:"음식"},
            {q:"연필로 글자를 ⬜", a:"쓰기"}, {q:"크레파스로 그리는 것", a:"그림"}, {q:"나를 부르는 호칭", a:"이름"}, {q:"텔레비전 연속극", a:"드라마"}, {q:"물이 아래로 ⬜", a:"흐르다"},

            // ㅣ
            {q:"내 명찰에 적힌 것", a:"이름"}, {q:"눈썹 위에 있는 얼굴", a:"이마"}, {q:"잘 때 덮는 것", a:"이불"}, {q:"다른 집으로 짐을 옮기는 것", a:"이사"}, {q:"아기가 먹는 죽", a:"이유식"},
            {q:"벌써, 다 끝난", a:"이미"}, {q:"지금부터", a:"이제"}, {q:"엄마의 여자 형제", a:"이모"}, {q:"그림이나 사진 (Image)", a:"이미지"}, {q:"새벽 풀잎에 맺히는 물방울", a:"이슬"}
        ];

        const bank_single = [
            // [1. ㄱ 받침]
            {q:"따뜻한 ⬜을 먹다.", a:"국", d:["굽","굼","궁"]}, {q:"맛있는 ⬜.", a:"떡", d:["떧","떵","떢"]}, {q:"요리하는 ⬜.", a:"부엌", d:["부억","부섟","부엍"]},
            {q:"창문 ⬜을 보다.", a:"밖", d:["박","밗","밬"]}, {q:"사과를 ⬜.", a:"깎다", d:["깍다","깍따","칵다"]}, {q:"바닥을 ⬜.", a:"닦다", d:["닥다","닧다","닭다"]},
            {q:"밥을 ⬜.", a:"볶다", d:["복다","봈다","볽다"]}, {q:"재료를 ⬜.", a:"섞다", d:["석다","섟다","섴다"]}, {q:"물고기를 ⬜.", a:"낚다", d:["낙다","낛다","낰다"]},
            {q:"신발 끈을 ⬜.", a:"묶다", d:["묵다","뭈다","묻다"]}, {q:"나뭇가지를 ⬜.", a:"꺾다", d:["꺽다","껈다","컥다"]}, {q:"밥을 ⬜.", a:"먹다", d:["망다","멍다","멌다"]},
            {q:"재미있는 ⬜.", a:"책", d:["채","챜","챙"]}, {q:"학교에 ⬜.", a:"가다", d:["싫다","까다","카다"]}, {q:"하늘이 ⬜.", a:"파랗다", d:["파라타","파랐다","파란다"]},

            // [2. ㄴ 받침]
            {q:"하늘에서 ⬜이 내리다.", a:"눈", d:["눙","눔","눋"]}, {q:"용⬜을 받다.", a:"돈", d:["동","돔","돋"]}, {q:"예쁜 ⬜.", a:"손", d:["솔","솜","송"]},
            {q:"친구에게 ⬜를 쓰다.", a:"편지", d:["펴지","평지","폄지"]}, {q:"양말을 ⬜.", a:"신다", d:["실다","심다","싣다"]}, {q:"아기를 ⬜.", a:"안다", d:["암다","알다","압다"]},
            {q:"문을 ⬜.", a:"연다", d:["염다","얀다","엽다"]}, {q:"친구를 ⬜.", a:"만나다", d:["말나다","맘나다","막나다"]}, {q:"우산이 ⬜.", a:"있다", d:["읻다","잇다","잊다"]},

            // [3. ㄷ/티읕(ㅌ) 계열]
            {q:"길을 ⬜.", a:"걷다", d:["것다","겆다","걸다"]}, {q:"방문을 ⬜.", a:"닫다", d:["닷다","닿다","닭다"]}, {q:"땅에 ⬜.", a:"묻다", d:["뭇다","뭍다","묵다"]},
            {q:"선생님께 ⬜.", a:"묻다", d:["뭇다","뭍다","묵다"]}, {q:"밥 먹는 ⬜.", a:"숟가락", d:["숫가락","술가락","숭가락"]}, {q:"노래를 ⬜.", a:"듣다", d:["듯다","듲다","들다"]},
            {q:"친구를 ⬜.", a:"믿다", d:["밋다","밑다","밀다"]}, {q:"선물을 ⬜.", a:"받다", d:["밭다","밧다","발다"]}, {q:"공부를 ⬜내다.", a:"끝", d:["끋","끗","끔"]},
            {q:"채소를 심은 ⬜.", a:"밭", d:["받","밧","발"]}, {q:"밥을 짓는 ⬜.", a:"솥", d:["솟","솓","솜"]}, {q:"의자 ⬜에 숨다.", a:"밑", d:["믿","밋","밀"]},
            {q:"수박의 ⬜은 초록색.", a:"겉", d:["걷","것","걸"]}, {q:"우리는 서로 ⬜.", a:"같다", d:["가타","갇다","갖다"]}, {q:"스티커를 ⬜.", a:"붙다", d:["붇다","붓다","불다"]},
            {q:"가까이 ⬜.", a:"붙다", d:["붇다","붓다","불다"]},

            // [4. ㄹ 받침]
            {q:"밤하늘의 ⬜.", a:"달", d:["닫","닷","당"]}, {q:"시원한 ⬜.", a:"물", d:["무","묻","묵"]}, {q:"뜨거운 ⬜.", a:"불", d:["부","붇","붓"]},
            {q:"얼굴을 보는 ⬜.", a:"거울", d:["거우","거운","거욷"]}, {q:"단풍이 드는 ⬜.", a:"가을", d:["가으","가은","가읃"]}, {q:"아파트에 ⬜.", a:"살다", d:["사다","삽다","삼다"]},
            {q:"친구와 ⬜.", a:"놀다", d:["노다","놉다","놈다"]}, {q:"정답을 ⬜.", a:"알다", d:["아다","압다","암다"]}, {q:"빗자루로 방을 ⬜.", a:"쓸다", d:["쓰다","씁다","씀다"]},
            {q:"창문을 ⬜.", a:"열다", d:["여다","엽다","염다"]},

            // [5. ㅁ 받침]
            {q:"맛있는 ⬜을 먹다.", a:"밤", d:["밥","방","발"]}, {q:"쿨쿨 ⬜을 자다.", a:"잠", d:["잡","장","잘"]}, {q:"튼튼한 ⬜.", a:"몸", d:["몹","몽","몰"]},
            {q:"꽃이 피는 ⬜.", a:"봄", d:["봅","봉","볼"]}, {q:"더운 ⬜.", a:"여름", d:["여릅","여릉","여를"]}, {q:"푹신한 ⬜.", a:"솜", d:["솝","송","솔"]},
            {q:"신나게 ⬜을 추다.", a:"춤", d:["춥","충","출"]}, {q:"그림을 ⬜.", a:"그림", d:["그립","그링","그릴"]},

            // [6. ㅂ/피읖(ㅍ) 계열]
            {q:"맛있는 ⬜.", a:"밥", d:["밤","방","밭"]}, {q:"⬜을 크게 벌리다.", a:"입", d:["이","임","익"]}, {q:"우리 ⬜.", a:"집", d:["짐","징","직"]},
            {q:"나무가 많은 ⬜.", a:"숲", d:["숩","숨","숟"]}, {q:"나뭇⬜이 떨어지다.", a:"잎", d:["입","임","익"]}, {q:"학교 ⬜.", a:"앞", d:["압","암","앗"]},
            {q:"내 ⬜에 앉아.", a:"옆", d:["엽","염","엿"]}, {q:"⬜이 아프다.", a:"무릎", d:["무릅","무름","무릍"]}, {q:"이불을 ⬜.", a:"덮다", d:["덥다","덤다","덜다"]},
            {q:"책을 ⬜.", a:"덮다", d:["덥다","덤다","덜다"]}, {q:"하늘이 ⬜.", a:"높다", d:["놉다","놊다","놑다"]}, {q:"가방이 ⬜.", a:"무겁다", d:["무겄다","무검다","무겋다"]},
            {q:"날씨가 ⬜.", a:"춥다", d:["춛다","춤다","충다"]}, {q:"날씨가 ⬜.", a:"덥다", d:["덛다","덤다","덩다"]}, {q:"친구를 ⬜.", a:"돕다", d:["독다","돔다","돌다"]},
            {q:"공을 ⬜.", a:"잡다", d:["잣다","작다","잠다"]}, {q:"옷을 ⬜.", a:"입다", d:["잎다","임다","잇다"]}, {q:"가방을 ⬜.", a:"매다", d:["메다","미다","마다"]},

            // [7. ㅇ 받침]
            {q:"흐르는 ⬜.", a:"강", d:["간","감","갈"]}, {q:"동그란 ⬜.", a:"공", d:["곤","곰","골"]}, {q:"내 ⬜을 청소하다.", a:"방", d:["반","밤","발"]},
            {q:"맛있는 ⬜.", a:"빵", d:["빤","빰","빨"]}, {q:"엄마 ⬜해요.", a:"사랑", d:["사란","사람","사랄"]}, {q:"귀여운 ⬜아지.", a:"강", d:["간","감","갈"]},

            // [8. ㅅ/쌍시옷(ㅆ) 계열]
            {q:"예쁜 ⬜을 입다.", a:"옷", d:["옫","옻","옥"]}, {q:"머리를 ⬜다.", a:"빗", d:["빚","빛","비"]}, {q:"사탕의 ⬜.", a:"맛", d:["맏","맞","맟"]},
            {q:"감기가 ⬜.", a:"낫다", d:["낳다","낮다","낯다"]}, {q:"밥을 ⬜.", a:"짓다", d:["짖다","짙다","짇다"]}, {q:"이름을 ⬜.", a:"짓다", d:["짖다","짙다","짇다"]},
            {q:"약을 ⬜.", a:"짓다", d:["짖다","짙다","짇다"]}, {q:"얼굴이 퉁퉁 ⬜.", a:"붓다", d:["붇다","붙다","불다"]}, {q:"물을 컵에 ⬜.", a:"붓다", d:["붇다","붙다","불다"]},
            {q:"숟가락으로 ⬜.", a:"젓다", d:["젖다","젗다","젇다"]}, {q:"고개를 ⬜.", a:"젓다", d:["젖다","젗다","젇다"]}, {q:"학교에 ⬜.", a:"있다", d:["잇다","잊다","빚다"]},
            {q:"집에 ⬜.", a:"갔다", d:["갓다","갇다","갖다"]}, {q:"과자를 ⬜.", a:"샀다", d:["삿다","싿다","샇다"]}, {q:"손을 ⬜.", a:"씻다", d:["싯다","싿다","씯다"]},
            {q:"글씨를 ⬜.", a:"쓰다", d:["스다","쑤다","씨다"]}, {q:"모자를 ⬜.", a:"쓰다", d:["스다","쑤다","씨다"]},

            // [9. ㅈ/치읓(ㅊ) 계열]
            {q:"⬜과 밤.", a:"낮", d:["낯","낫","낳"]}, {q:"⬜잠을 자다.", a:"낮", d:["낯","낫","낳"]}, {q:"⬜을 갚다.", a:"빚", d:["빗","빛","비"]},
            {q:"만두를 ⬜.", a:"빚다", d:["빗다","빛다","빋다"]}, {q:"보물을 ⬜.", a:"찾다", d:["찻다","찯다","챃다"]}, {q:"약속을 ⬜.", a:"잊다", d:["잇다","잏다","읻다"]},
            {q:"시간에 ⬜.", a:"늦다", d:["늣다","늗다","늘다"]}, {q:"옷이 물에 ⬜.", a:"젖다", d:["젓다","젗다","젇다"]}, {q:"강아지가 멍멍 ⬜.", a:"짖다", d:["짓다","짙다","짇다"]},
            {q:"도둑을 ⬜.", a:"쫓다", d:["쫒다","쫏다","쫃다"]}, {q:"예쁜 ⬜이 피다.", a:"꽃", d:["꼿","꽂","꽅"]}, {q:"반짝반짝 ⬜이 나다.", a:"빛", d:["빗","빚","빋"]},
            {q:"햇⬜이 비치다.", a:"빛", d:["빗","빚","빋"]}, {q:"얼굴 ⬜이 좋다.", a:"빛", d:["빗","빚","빋"]}, {q:"나무 ⬜.", a:"숯", d:["숫","숟","숭"]},
            {q:"머리핀을 ⬜.", a:"꽂다", d:["꼿다","꼽다","꽅다"]},

            // [10. ㅎ 계열]
            {q:"기분이 ⬜.", a:"좋다", d:["조타","좃다","졿다"]}, {q:"가방에 책을 ⬜.", a:"넣다", d:["너타","넛다","넏다"]}, {q:"식탁에 숟가락을 ⬜.", a:"놓다", d:["노타","놏다","놌다"]},
            {q:"걱정을 ⬜.", a:"놓다", d:["노타","놏다","놌다"]}, {q:"아기를 ⬜.", a:"낳다", d:["낫다","낮다","낯다"]}, {q:"마늘을 ⬜.", a:"찧다", d:["찟다","찓다","찌다"]},
            {q:"하⬜다.", a:"하얗다", d:["하야타","하얐다","하얃다"]}, {q:"파⬜다.", a:"파랗다", d:["파라타","파랐다","파랃다"]}, {q:"노⬜다.", a:"노랗다", d:["노라타","노랐다","노랃다"]},
            {q:"빨⬜다.", a:"빨갛다", d:["빨가타","빨갔다","빨갇다"]},

            // [11. 추가 혼동하기 쉬운 단어들]
            {q:"⬜가 튼튼하다.", a:"이", d:["이","니","리"]}, {q:"하늘을 ⬜.", a:"날다", d:["나다","납다","남다"]}, {q:"땀이 ⬜.", a:"나다", d:["날다","납다","남다"]},
            {q:"잠이 ⬜.", a:"오다", d:["오다","요다","우다"]}, {q:"김치를 ⬜.", a:"담그다", d:["당그다","담크다","담다"]}, {q:"문을 ⬜.", a:"잠그다", d:["장그다","잠크다","잠다"]}
        ];

        const bank_complex = [
            // [1. ㄳ]
            {q:"내 ⬜을 챙기다.", a:"몫", d:["목","몴","몪"]}, {q:"할 일을 다 하는 ⬜.", a:"몫", d:["목","몴","몪"]}, {q:"하루 일한 ⬜.", a:"삯", d:["삭","삮","살"]},
            {q:"수고한 ⬜을 받다.", a:"삯", d:["삭","삮","살"]}, {q:"⬜을 잃고 쳐다보다.", a:"넋", d:["넉","넔","넠"]}, {q:"⬜이 나가다.", a:"넋", d:["넉","넔","넠"]},

            // [2. ㄵ]
            {q:"의자에 ⬜.", a:"앉다", d:["안다","않다","앋다"]}, {q:"바닥에 털썩 ⬜.", a:"앉다", d:["안다","않다","앋다"]}, {q:"내 옆자리에 ⬜.", a:"앉다", d:["안다","않다","앋다"]},
            {q:"친구와 나란히 ⬜.", a:"앉다", d:["안다","않다","앋다"]}, {q:"자세 보정 의자에 ⬜.", a:"앉다", d:["안다","않다","앋다"]}, {q:"선반에 물건을 ⬜.", a:"얹다", d:["언다","얻다","엄다"]},
            {q:"머리에 수건을 ⬜.", a:"얹다", d:["언다","얻다","엄다"]}, {q:"밥 위에 반찬을 ⬜.", a:"얹다", d:["언다","얻다","엄다"]}, {q:"어깨에 손을 ⬜.", a:"얹다", d:["언다","얻다","엄다"]},
            {q:"케이크에 과일을 ⬜.", a:"얹다", d:["언다","얻다","엄다"]},

            // [3. ㄶ]
            {q:"사람이 정말 ⬜.", a:"많다", d:["만다","맏다","말다"]}, {q:"숙제가 너무 ⬜.", a:"많다", d:["만다","맏다","말다"]}, {q:"걱정이 ⬜.", a:"많다", d:["만다","맏다","말다"]},
            {q:"가위로 줄을 ⬜.", a:"끊다", d:["끏다","끋다","끌다"]}, {q:"전화를 뚝 ⬜.", a:"끊다", d:["끏다","끋다","끌다"]}, {q:"담배를 ⬜.", a:"끊다", d:["끏다","끋다","끌다"]},
            {q:"기차표를 ⬜.", a:"끊다", d:["끏다","끋다","끌다"]}, {q:"밥을 먹지 ⬜.", a:"않다", d:["앉다","앋다","알다"]}, {q:"숙제를 하지 ⬜.", a:"않다", d:["앉다","앋다","알다"]},
            {q:"거짓말을 하지 ⬜.", a:"않다", d:["앓다","앋다","알다"]}, {q:"나는 몸이 ⬜.", a:"괜찮다", d:["괜찬다","괜찭다","괜챦다"]}, {q:"성격이 참 ⬜.", a:"괜찮다", d:["괜찬다","괜찭다","괜챦다"]},
            {q:"청소하기 ⬜.", a:"귀찮다", d:["귀찬다","귀찭다","귀챦다"]}, {q:"씻기가 너무 ⬜.", a:"귀찮다", d:["귀찬다","귀찭다","귀챦다"]}, {q:"할머니께서 ⬜.", a:"편찮다", d:["편찬다","편찭다","편챦다"]},

            // [4. ㄺ]
            {q:"알을 낳는 ⬜.", a:"닭", d:["닥","달","닧"]}, {q:"맛있는 ⬜고기.", a:"닭", d:["닥","달","닧"]}, {q:"치킨은 ⬜을 튀긴 것.", a:"닭", d:["닥","달","닧"]},
            {q:"동화책을 ⬜.", a:"읽다", d:["일다","익다","읿다"]}, {q:"소리 내어 ⬜.", a:"읽다", d:["일다","익다","읿다"]}, {q:"친구의 마음을 ⬜.", a:"읽다", d:["일다","익다","읿다"]},
            {q:"가을 하늘이 ⬜.", a:"맑다", d:["말다","막다","맊다"]}, {q:"계곡 물이 ⬜.", a:"맑다", d:["말다","막다","맊다"]}, {q:"머리가 ⬜.", a:"맑다", d:["말다","막다","맊다"]},
            {q:"사람은 누구나 ⬜.", a:"늙다", d:["늑다","늘다","늚다"]}, {q:"단풍이 ⬜.", a:"붉다", d:["북다","불다","붚다"]}, {q:"얼굴이 ⬜.", a:"붉다", d:["북다","불다","붚다"]},
            {q:"보름달이 ⬜.", a:"밝다", d:["발다","박다","밠다"]}, {q:"표정이 ⬜.", a:"밝다", d:["발다","박다","밠다"]}, {q:"오래된 집이 ⬜.", a:"낡다", d:["낙다","날다","낛다"]},
            {q:"옷이 많이 ⬜.", a:"낡다", d:["낙다","날다","낛다"]}, {q:"손가락이 ⬜.", a:"굵다", d:["국다","굴다","굯다"]}, {q:"빗줄기가 ⬜.", a:"굵다", d:["국다","굴다","굯다"]},
            {q:"목소리가 ⬜.", a:"굵다", d:["국다","굴다","굯다"]}, {q:"등을 시원하게 ⬜.", a:"긁다", d:["극다","글다","긇다"]}, {q:"가려운 곳을 ⬜.", a:"긁다", d:["극다","글다","긇다"]},
            {q:"복권을 ⬜.", a:"긁다", d:["극다","글다","긇다"]},

            // [5. ㄻ]
            {q:"행복한 ⬜.", a:"삶", d:["삼","살","삷"]}, {q:"배움을 통한 ⬜.", a:"앎", d:["암","알","앏"]}, {q:"달걀을 ⬜.", a:"삶다", d:["삼다","삻다","삽다"]},
            {q:"국수를 ⬜.", a:"삶다", d:["삼다","삻다","삽다"]}, {q:"배가 고파서 ⬜.", a:"굶다", d:["굼다","굷다","굽다"]}, {q:"하루 종일 ⬜.", a:"굶다", d:["굼다","굴다","굽다"]},
            {q:"나이가 ⬜.", a:"젊다", d:["점다","절다","젋다"]}, {q:"생각이 ⬜.", a:"젊다", d:["점다","절다","젋다"]}, {q:"짐을 ⬜.", a:"옮기다", d:["옴기다","옰기다","옹기다"]},
            {q:"병을 ⬜.", a:"옮기다", d:["옴기다","옯기다","옹기다"]}, {q:"아빠를 ⬜.", a:"닮다", d:["담다","달다","답다"]}, {q:"서로 ⬜.", a:"닮다", d:["담다","달다","답다"]},

            // [6. ㄼ]
            {q:"숫자 8은 ⬜.", a:"여덟", d:["여덣","여덞","여넙"]}, {q:"나는 ⬜ 살이다.", a:"여덟", d:["여덣","여덞","여넙"]}, {q:"운동장이 ⬜.", a:"넓다", d:["넗다","넙다","넔다"]},
            {q:"마음이 ⬜.", a:"넓다", d:["덟다","넙다","넔다"]}, {q:"거실이 참 ⬜.", a:"넓다", d:["널다","넙다","넔다"]}, {q:"바지가 너무 ⬜.", a:"짧다", d:["짤다","짭다","짋다"]},
            {q:"머리카락이 ⬜.", a:"짧다", d:["짤다","짭다","짋다"]}, {q:"시간이 ⬜.", a:"짧다", d:["짤다","짭다","짋다"]}, {q:"땅을 ⬜.", a:"밟다", d:["밥다","발다","밞다"]},
            {q:"브레이크를 ⬜.", a:"밟다", d:["밥다","발다","밞다"]}, {q:"남의 발을 ⬜.", a:"밟다", d:["밥다","발다","밞다"]}, {q:"종이가 ⬜.", a:"얇다", d:["얄다","얍다","얋다"]},
            {q:"여름 옷은 ⬜.", a:"얇다", d:["얄다","얍다","얋다"]}, {q:"책 두께가 ⬜.", a:"얇다", d:["얄다","얍다","얋다"]}, {q:"덜 익은 감이 ⬜.", a:"떫다", d:["떯다","떱다","떪다"]},

            // [8. ㄾ]
            {q:"사탕을 ⬜.", a:"핥다", d:["핣다","핟다","합다"]}, {q:"강아지가 손을 ⬜.", a:"핥다", d:["핣다","핟다","합다"]}, {q:"아이스크림을 ⬜.", a:"핥다", d:["핣다","핟다","합다"]},
            {q:"내용을 대충 ⬜.", a:"훑다", d:["훏다","훋다","훗다"]}, {q:"위아래로 ⬜.", a:"훑다", d:["훏다","훋다","훗다"]},

            // [9. ㄿ]
            {q:"시를 소리 내어 ⬜.", a:"읊다", d:["읍다","을다","읇다"]}, {q:"시조를 ⬜.", a:"읊다", d:["읍다","을다","읇다"]}, {q:"가사를 조용히 ⬜.", a:"읊다", d:["읍다","을다","읇다"]},

            // [10. ㅀ]
            {q:"길을 ⬜.", a:"잃다", d:["일다","잊다","임다"]}, {q:"지갑을 ⬜.", a:"잃다", d:["일다","잊다","임다"]}, {q:"친구를 ⬜.", a:"잃다", d:["일다","잊다","임다"]},
            {q:"기회를 ⬜.", a:"잃다", d:["일다","잊다","임다"]}, {q:"공부하기 ⬜.", a:"싫다", d:["싦다","싷다","심다"]}, {q:"콩을 먹기 ⬜.", a:"싫다", d:["싦다","싷다","심다"]},
            {q:"나쁜 냄새가 ⬜.", a:"싫다", d:["싦다","싷다","심다"]}, {q:"송곳으로 구멍을 ⬜.", a:"뚫다", d:["뚤다","뚣다","뚬다"]}, {q:"막힌 길을 ⬜.", a:"뚫다", d:["뚤다","뚦다","뚬다"]},
            {q:"귀를 ⬜.", a:"뚫다", d:["뚤다","뚣다","뚦다"]}, {q:"찌개가 보글보글 ⬜.", a:"끓다", d:["끌다","끋다","끎다"]}, {q:"라면 물이 ⬜.", a:"끓다", d:["끌다","끋다","끎다"]},
            {q:"커피 물을 ⬜.", a:"끓다", d:["끌다","끋다","끎다"]}, {q:"감기를 심하게 ⬜.", a:"앓다", d:["알다","앏다","암다"]}, {q:"배탈을 ⬜.", a:"앓다", d:["알다","앋다","앏다"]},
            {q:"가슴을 ⬜.", a:"앓다", d:["알다","앋다","앏다"]},

            // [11. ㅄ]
            {q:"시간이 ⬜.", a:"없다", d:["업다","엽다","엄다"]}, {q:"돈이 하나도 ⬜.", a:"없다", d:["업다","엽다","엄다"]}, {q:"집에 아무도 ⬜.", a:"없다", d:["업다","엽다","엄다"]},
            {q:"재미가 ⬜.", a:"없다", d:["업다","얾다","엄다"]}, {q:"할 말이 ⬜.", a:"없다", d:["업다","얾다","엄다"]}, {q:"기운이 ⬜.", a:"없다", d:["업다","얾다","엄다"]},
            {q:"정답이 ⬜.", a:"없다", d:["업다","얾다","엄다"]}, {q:"물건의 ⬜.", a:"값", d:["갑","갚","갋"]}, {q:"음식 ⬜을 내다.", a:"값", d:["갑","갚","갋"]},
            {q:"⬜이 비싸다.", a:"값", d:["갑","갚","갋"]}, {q:"옷 ⬜이 싸다.", a:"값", d:["갑","갚","갋"]}, {q:"강아지가 참 ⬜.", a:"가엾다", d:["가엽다","가업다","가없다"]},
            {q:"배고픈 아이가 ⬜.", a:"가엾다", d:["가엽다","가업다","가없다"]}
        ];

        const bank_life_eng = [
            // 1. 인사 및 표현
            {q:"안녕하세요", a:"Hello"}, {q:"안녕 (친구)", a:"Hi"}, {q:"잘 가", a:"Bye"}, {q:"고맙습니다", a:"Thank you"}, {q:"미안합니다", a:"Sorry"}, {q:"실례합니다", a:"Excuse me"},
            {q:"네 / 좋아요", a:"Yes"}, {q:"아니요 / 싫어요", a:"No"}, {q:"제발 / ~해주세요", a:"Please"}, {q:"알겠어", a:"Okay"}, {q:"좋은 아침", a:"Good morning"}, {q:"만나서 반가워", a:"Nice to meet you"},
            // 2. 표지판/안전
            {q:"멈춤", a:"Stop"}, {q:"가다 / 출발", a:"Go"}, {q:"비상구 / 나가는 곳", a:"Exit"}, {q:"안", a:"In"}, {q:"밖 (Out)", a:"Out"}, {q:"미세요", a:"Push"},
            {q:"당기세요", a:"Pull"}, {q:"화장실", a:"Toilet"}, {q:"위험", a:"Danger"}, {q:"열림 / 영업 중", a:"Open"}, {q:"닫힘 / 영업 종료", a:"Close"}, {q:"조심해", a:"Watch out"},
            // 3. 식당/카페
            {q:"차림표", a:"Menu"}, {q:"커피", a:"Coffee"}, {q:"물", a:"Water"}, {q:"주스", a:"Juice"}, {q:"뜨거운", a:"Hot"}, {q:"차가운 / 얼음", a:"Ice"},
            {q:"포장", a:"Take out"}, {q:"크기", a:"Size"}, {q:"닭고기", a:"Chicken"}, {q:"피자", a:"Pizza"}, {q:"우유", a:"Milk"}, {q:"빵", a:"Bread"},
            // 4. 디지털/게임
            {q:"무선 인터넷", a:"WiFi"}, {q:"데이터", a:"Data"}, {q:"켜짐", a:"On"}, {q:"꺼짐", a:"Off"}, {q:"시작", a:"Start"}, {q:"게임 끝", a:"Game Over"},
            {q:"아이디", a:"ID"}, {q:"비밀번호", a:"Password"}, {q:"건너뛰기", a:"Skip"}, {q:"검색", a:"Search"}, {q:"승리", a:"Win"}, {q:"패배", a:"Lose"},
            // 5. 쇼핑/물건
            {q:"할인", a:"Sale"}, {q:"가격", a:"Price"}, {q:"새로운 것", a:"New"}, {q:"최고 / 인기", a:"Best"}, {q:"매진 (다 팔림)", a:"Sold out"}, {q:"현금", a:"Cash"},
            {q:"카드", a:"Card"}, {q:"가방 / 봉투", a:"Bag"}, {q:"선물", a:"Gift"}, {q:"공짜 / 자유", a:"Free"}, {q:"시장 / 마트", a:"Market"}, {q:"상자", a:"Box"},
            // 6. 장소/이동
            {q:"버스", a:"Bus"}, {q:"택시", a:"Taxi"}, {q:"지하철", a:"Subway"}, {q:"승강기", a:"Elevator"}, {q:"학교", a:"School"}, {q:"집", a:"Home"},
            {q:"마트 / 가게", a:"Store"}, {q:"공원", a:"Park"}, {q:"주차장", a:"Parking"}, {q:"표", a:"Ticket"}, {q:"방", a:"Room"}, {q:"길", a:"Road"},
            // 7. 나/사람
            {q:"나", a:"I"}, {q:"너 / 당신", a:"You"}, {q:"이름", a:"Name"}, {q:"친구", a:"Friend"}, {q:"선생님", a:"Teacher"}, {q:"학생", a:"Student"},
            {q:"엄마", a:"Mom"}, {q:"아빠", a:"Dad"}, {q:"가족", a:"Family"}, {q:"아기", a:"Baby"}, {q:"남자", a:"Man"}, {q:"여자", a:"Woman"},
            // 8. 감정/상태
            {q:"행복한", a:"Happy"}, {q:"슬픈", a:"Sad"}, {q:"좋은", a:"Good"}, {q:"나쁜", a:"Bad"}, {q:"사랑", a:"Love"}, {q:"화난", a:"Angry"},
            {q:"배고픈", a:"Hungry"}, {q:"목마른", a:"Thirsty"}, {q:"쉬운", a:"Easy"}, {q:"어려운", a:"Hard"}, {q:"아픈", a:"Sick"}, {q:"피곤한", a:"Tired"},
            // 9. 숫자/시간/날짜
            {q:"하나 (1)", a:"One"}, {q:"둘 (2)", a:"Two"}, {q:"셋 (3)", a:"Three"}, {q:"열 (10)", a:"Ten"}, {q:"시간", a:"Time"}, {q:"낮 / 하루", a:"Day"},
            {q:"밤", a:"Night"}, {q:"아침", a:"Morning"}, {q:"지금", a:"Now"}, {q:"날짜", a:"Date"}, {q:"주말", a:"Weekend"}, {q:"년 (해)", a:"Year"},
            // 10. 색깔/자연/기타
            {q:"빨강", a:"Red"}, {q:"파랑", a:"Blue"}, {q:"노랑", a:"Yellow"}, {q:"초록", a:"Green"}, {q:"검정", a:"Black"}, {q:"하양", a:"White"},
            {q:"큰", a:"Big"}, {q:"작은", a:"Small"}, {q:"해", a:"Sun"}, {q:"달", a:"Moon"}, {q:"별", a:"Star"}, {q:"비", a:"Rain"}
        ];

        const R_DATA = {
            locations: ["국립과학관", "동물원", "놀이공원", "식물원", "역사박물관", "아쿠아리움", "민속촌", "자동차박물관"],
            items: ["도시락", "물", "모자", "돋보기", "우산", "필기도구", "수첩", "카메라", "간식", "멀미약", "휴지", "돗자리"],
            weekdays: ["월", "화", "수", "목", "금", "토", "일"],
            cities: ["서울", "부산", "대구", "대전", "광주", "울산", "강릉", "전주", "경주", "포항"],
            names: ["김철수", "이영희", "김영환", "윤지호", "장영재", "윤준영", "허찬", "박지훈"],
            medicines: ["감기약", "두통약", "소화제", "비염약", "안약", "피부약"],
            warnings: ["졸음", "복통", "두통", "발진", "어지러움", "가려움", "속쓰림"],
            storage: ["실온", "냉장고", "서늘한 곳", "빛이 없는 곳", "건조한 곳"],
            foods: ["비빔밥", "돈가스", "김치찌개", "불고기", "짜장면", "볶음밥", "햄버거", "치킨"],
            movies: ["슈퍼히어로", "공룡의 모험", "우주 여행", "탐정 코난", "겨울 왕국", "아바타", "토이스토리"],
            lostItems: ["지갑", "휴대폰", "가방", "우산", "안경", "모자", "카드", "이어폰", "인형", "열쇠"]
        };
        
        const SCHEDULE_DATA = {
            STUDENT: { title: "🧑‍🎓 이학생의 하루", items: [ { t: "07:00", a: "기상" }, { t: "08:00", a: "등교" }, { t: "09:00", a: "1교시" }, { t: "12:30", a: "점심" }, { t: "14:30", a: "하교" }, { t: "16:00", a: "학원" }, { t: "19:00", a: "저녁" }, { t: "21:30", a: "취침" } ] },
            WORKER: { title: "👔 김대리의 하루", items: [ { t: "06:30", a: "기상" }, { t: "08:50", a: "출근" }, { t: "12:00", a: "점심" }, { t: "13:00", a: "업무" }, { t: "18:00", a: "퇴근" }, { t: "19:30", a: "저녁" }, { t: "21:00", a: "운동" }, { t: "00:00", a: "취침" } ] },
            HOUSEWIFE: { title: "🏠 박주부의 하루", items: [ { t: "06:00", a: "기상" }, { t: "07:00", a: "아침준비" }, { t: "09:00", a: "청소" }, { t: "12:00", a: "점심" }, { t: "14:00", a: "장보기" }, { t: "17:00", a: "저녁준비" }, { t: "20:00", a: "육아퇴근" }, { t: "23:00", a: "취침" } ] },
            FREELANCER: { title: "💻 프리랜서의 하루", items: [ { t: "09:00", a: "기상" }, { t: "10:00", a: "이메일확인" }, { t: "11:00", a: "작업시작" }, { t: "14:00", a: "늦은점심" }, { t: "15:00", a: "미팅" }, { t: "18:00", a: "작업퇴근" }, { t: "21:00", a: "야간작업" }, { t: "02:00", a: "취침" } ] },
            ELEMENTARY_LOW: { title: "🎒 1학년의 하루", items: [ { t: "07:30", a: "일어나기" }, { t: "08:30", a: "학교가기" }, { t: "09:00", a: "공부하기" }, { t: "12:00", a: "급식먹기" }, { t: "13:00", a: "집에가기" }, { t: "14:00", a: "놀이터" }, { t: "18:00", a: "저녁먹기" }, { t: "21:00", a: "잠자기" } ] },
            HIGH_SCHOOL: { title: "📚 고3의 하루", items: [ { t: "06:00", a: "기상" }, { t: "07:00", a: "등교" }, { t: "08:00", a: "0교시" }, { t: "13:00", a: "점심시간" }, { t: "18:00", a: "석식" }, { t: "19:00", a: "야자" }, { t: "23:00", a: "하교" }, { t: "01:00", a: "취침" } ] }
        };

        const bank_recycle = [
            {q:"택배 상자(테이프제거)", a:"종이"}, {q:"신문지", a:"종이"}, {q:"우유팩(물로 헹굼)", a:"종이"}, {q:"다 쓴 공책(스프링 제거)", a:"종이"},
            {q:"투명 커피컵", a:"플라스틱"}, {q:"생수병(라벨제거)", a:"플라스틱"}, {q:"샴푸통", a:"플라스틱"}, {q:"요구르트병", a:"플라스틱"},
            {q:"참치 캔", a:"캔"}, {q:"콜라 캔", a:"캔"}, {q:"부탄가스(구멍뚫음)", a:"캔"}, {q:"소주병", a:"유리"}, {q:"맥주병", a:"유리"}, {q:"참기름 병", a:"유리"},
            {q:"과자 봉지", a:"비닐"}, {q:"라면 봉지", a:"비닐"}, {q:"검정 비닐봉투", a:"비닐"}, {q:"치킨 뼈", a:"일반쓰레기"}, {q:"물티슈", a:"일반쓰레기"},
            {q:"깨진 유리", a:"일반쓰레기"}, {q:"바나나 껍질", a:"음식물"}, {q:"수박 껍질", a:"음식물"}, {q:"남은 밥", a:"음식물"}, {q:"컵라면 용기(국물자국)", a:"일반쓰레기"},
            {q:"컵라면 용기(깨끗함)", a:"스티로폼"}, {q:"택배 스티로폼", a:"스티로폼"}
        ];

        const RANK_SCORE = { 'SSS': 100, 'SS': 95, 'S': 90, 'A+': 88, 'A': 85, 'B+': 83, 'B': 80, 'C+': 75, 'C': 70, '-': 0 };
        const TARGET_SCORE = 50;
   // ==========================================
        // [3] 문제 생성 로직: 국어 (안내문 독해)
        // Code 2의 모든 시나리오(기초 0~5, 심화 0~9) 완벽 이식
        // ==========================================

        // [3-1] 안내문 독해 (기초) - Code 2 generateReadingQuiz() 이식
        function generateReadingQuiz() {
            const scenarioType = getRandomInt(0, 5); // 0:체험학습, 1:기차표, 2:병원, 3:약봉투, 4:분실물, 5:영화표
            ...
            if (scenarioType === 0) {
                ...
                content = `
                    <div class="text-left space-y-2">
                        <p><strong>1. 일시:</strong> ${date} (${day}) ${timeStart} ~ ${timeEnd}</p>
                        <p><strong>2. 장소:</strong> ${dest} (버스 ${busNum}호차)</p>
                        <p><strong>3. 준비물:</strong> ${item1}, ${item2}, 운동화</p>
                        <p><strong>4. 모이는 시간:</strong> ${timeMeet}까지 교실로</p>
                        <div class="mt-4 p-2 bg-yellow-50/50 rounded text-sm border border-yellow-200">
                            <p class="text-gray-600 font-bold">* 비가 오면 '체육관'에서 진행합니다.</p>
                        </div>
                    </div>`;

                const qPool = [
                    { q: "체험학습 장소는 어디인가요?", a: dest, t: 'array', src: R_DATA.locations },
                    { q: "무슨 요일에 가나요?", a: day + "요일", t: 'array', src: R_DATA.weekdays.map(d=>d+"요일") },
                    { q: "꼭 가져가야 할 물건은?", a: item1, t: 'array', src: R_DATA.items },
                    { q: "몇 시까지 모여야 하나요?", a: timeMeet, t: 'time' },
                    { q: "체험학습이 끝나는 시간은?", a: timeEnd, t: 'time' },
                    { q: "타고 가는 버스는 몇 호차인가요?", a: busNum+"호차", t: 'text_num' },
                    { q: "비가 오면 어디서 진행하나요?", a: "체육관", t: 'array', src: ["운동장", "교실", "식당", "강당"] }
                ];
            ...
            // --- 시나리오 1: 기차표 (KTX) ---
                ...
                content = `
                    <div class="border-2 border-dashed border-gray-400 p-4 rounded bg-gray-50 text-black">
                        ...
                        <div class="space-y-1">
                            ...
                            <p><strong>출발:</strong> ${tDep} / <strong>도착:</strong> ${tArr}</p>
                            <p><strong>좌석:</strong> ${car}호차 ${seat}</p>
                            <p><strong>운임:</strong> ${priceStr}</p>
                        </div>
                    </div>`;

                const qPool = [
                    { q: "출발하는 도시는?", a: dep, t: 'array', src: R_DATA.cities },
                    { q: "도착하는 도시는?", a: arr, t: 'array', src: R_DATA.cities },
                    { q: "기차가 출발하는 시간은?", a: tDep, t: 'time' },
                    { q: "기차에 도착하는 시간은?", a: tArr, t: 'time' },
                    { q: "몇 호차에 타야 하나요?", a: car+"호차", t: 'text_num' },
                    { q: "좌석 번호는 무엇인가요?", a: seat, t: 'array', src: [getRandomInt(1,15)+"A", getRandomInt(1,15)+"B", getRandomInt(1,15)+"C"] },
                    { q: "기차표 가격은 얼마인가요?", a: priceStr, t: 'text_num' } 
                ];
                const q = getRand(qPool);
                ...
                if(q.q.includes("가격")) {
                    ...
                    while(pSet.size < 4 && loop < 50) { 
                        pSet.add((price + getRandomInt(-10, 10)*1000).toLocaleString() + "원");
                        loop++;
                    }
                    while(pSet.size < 4) pSet.add((price + pSet.size * 1000 + 5000).toLocaleString() + "원");
            ...
            // --- 시나리오 2: 약 봉투 ---
                ...
                const timing = ["식사 후 30분", "식사 전 30분", "식사 직후", "자기 전"][getRandomInt(0, 3)];
                ...
                content = `
                    <div class="bg-white border border-gray-200 p-4 rounded text-left shadow-sm">
                        <p class="font-bold border-b-2 border-gray-800 mb-3 pb-1 text-lg">${name} 님 (${med})</p>
                        <div class="space-y-1 text-sm">
                            <p><strong>- 용법:</strong> 1일 ${perDay}회, ${days}일분</p>
                            <p><strong>- 시간:</strong> ${timing} 복용</p>
                            <p><strong>- 보관:</strong> ${store}</p>
                            <p class="text-red-600 font-bold mt-2">※ 주의: ${warn} 증상 가능</p>
                        </div>
                    </div>`;

                const qPool = [
                    { q: "이 약은 누구의 것인가요?", a: name, t: 'array', src: R_DATA.names },
                    { q: "하루에 몇 번 먹나요?", a: perDay+"회", t: 'text_num' },
                    { q: "며칠 동안 먹어야 하나요?", a: days+"일분", t: 'text_num' },
                    { q: "언제 먹어야 하나요?", a: timing, t: 'array', src: ["식사 후 30분", "식사 전 30분", "식사 직후", "자기 전"] },
                    { q: "어디에 보관해야 하나요?", a: store, t: 'array', src: R_DATA.storage },
                    { q: "주의해야 할 증상은?", a: warn, t: 'array', src: R_DATA.warnings }
                ];
                const q = getRand(qPool);
                answer = q.a; options = getDistractors(q.a, q.t, q.src); qList = q.q;
            }
            ...
            // --- 시나리오 3: 영화 입장권 ---
                ...
                const fullLocAnswer = `${floor}층 ${theater}관`;
                const fullLocOptions = [
                    fullLocAnswer,                                      
                    `${floor}층 ${theater === 9 ? 8 : theater + 1}관`,  
                    `${floor === 7 ? 6 : floor + 1}층 ${theater}관`,    
                    `${getRandomInt(1, 7)}층 ${getRandomInt(1, 9)}관`   
                ];
                ...
                content = `
                    <div class="text-left bg-gray-900 p-4 rounded border border-gray-700 text-white">
                        ...
                        <div class="space-y-1 text-gray-200">
                            <p><strong>영화:</strong> ${movie}</p>
                            <p><strong>상영관:</strong> <span class="text-green-400 font-bold">${theater}관</span> (${floor}층)</p>
                            <p><strong>시간:</strong> ${tStart} 시작</p>
                            <p><strong>인원:</strong> 성인 ${adults}명, 청소년 ${kids}명</p>
                            <p class="mt-2 text-sm text-gray-400">※ 주차: 3시간 무료</p>
                        </div>
                    </div>`;

                const qPool = [
                    { q: "어떤 영화를 보나요?", a: movie, t: 'array', src: R_DATA.movies },
                    { q: "상영관은 몇 관인가요?", a: theater+"관", t: 'text_num' },      
                    { q: "상영관은 몇 층에 있나요?", a: floor+"층", t: 'text_num' },       
                    { q: "상영관은 몇 층, 몇 관인가요?", a: fullLocAnswer, t: 'array', src: fullLocOptions }, 
                    { q: "영화 시작 시간은?", a: tStart, t: 'time' },
                    { q: "관람하는 사람은 모두 몇 명?", a: totalP+"명", t: 'text_num' },
                    { q: "성인은 몇 명인가요?", a: adults+"명", t: 'text_num' }
                ];
                
                const q = getRand(qPool);
                answer = q.a; 
                if(q.src && q.q.includes("몇 층, 몇 관")) { options = shuffleArray(q.src); } 
            ...
            // --- 시나리오 4: 분실물 ---
                ...
                const color = getRand(["빨간색", "파란색", "검은색", "노란색", "흰색", "초록색"]);
                const place = getRand(["운동장", "급식실", "도서관", "음악실", "화장실", "복도"]);
                const owner = getRand(R_DATA.names);
                const grade = getRandomInt(1, 6);
                const classNum = getRandomInt(1, 5);
                const storageList = ["행정실", "교무실", "보건실", "경비실", "방송실", "상담실"];
                const currentStorage = getRand(storageList);

                title = "주인을 찾습니다";
                content = `
                    <div class="text-left bg-yellow-50 p-4 rounded border border-yellow-200">
                        <h3 class="text-center font-bold text-xl mb-4 text-yellow-800">분실물 습득</h3>
                        <div class="space-y-2 text-gray-800">
                            <p><span class="text-gray-500 font-bold">물건:</span> ${color} ${item}</p>
                            <p><span class="text-gray-500 font-bold">발견 장소:</span> ${place}</p>
                            <p><span class="text-gray-500 font-bold">보관 중:</span> ${currentStorage}</p> 
                            <p><span class="text-gray-500 font-bold">연락처:</span> ${grade}학년 ${classNum}반 ${owner}</p>
                        </div>
                    </div>`;

                const qPool = [
                    { q: "잃어버린 물건은 무엇인가요?", a: item, t: 'array', src: R_DATA.lostItems },
                    { q: "물건의 색깔은?", a: color, t: 'array', src: ["빨간색", "파란색", "검은색", "노란색"] },
                    { q: "어디서 주웠나요?", a: place, t: 'array', src: ["운동장", "급식실", "도서관", "화장실"] },
                    { q: "주인의 학년과 반은?", a: `${grade}학년 ${classNum}반`, t: 'text_num' }, 
                    { q: "지금 어디에 보관 중인가요?", a: currentStorage, t: 'array', src: storageList },
                    { q: "연락해야 할 학생 이름은?", a: owner, t: 'array', src: R_DATA.names }
                ];
                const q = getRand(qPool);
                ...
                if(q.q.includes("학년")) {
                     ...
                     while(oSet.size < 4 && loop < 50) { 
                         oSet.add(`${getRandomInt(1,6)}학년 ${getRandomInt(1,5)}반`);
                         loop++;
                     }
                     while(oSet.size < 4) oSet.add(`${getRandomInt(1,6)}학년 ${getRandomInt(6,9)}반`);
            ...
            // --- 시나리오 5: 병원 안내 ---
                ...
                const hName = getRand(["행복", "사랑", "튼튼", "서울", "우리", "건강", "소망", "바른"]);
                const type = getRand(["내과", "소아과", "이비인후과", "치과", "안과", "가정의학과"]);
                ...
                const closedDay = getRand(["일요일", "일요일/공휴일", "수요일", "목요일"]);

                title = `${fullName} 진료 안내`;
                content = `
                    <div class="text-left bg-white p-4 rounded border border-green-200 shadow-sm">
                        ...
                        <ul class="space-y-2 text-sm text-gray-700">
                            <li><span class="font-bold">평일:</span> ${openTime} ~ ${closeTime}</li>
                            <li><span class="font-bold">점심시간:</span> ${lunchStart} ~ ${lunchEnd}</li>
                            <li><span class="font-bold">토요일:</span> ${openTime} ~ ${satClose}</li>
                            <li><span class="text-red-500 font-bold">휴진:</span> ${closedDay}</li>
                        </ul>
                    </div>`;

                const qPool = [
                    { q: "병원의 이름은 무엇인가요?", a: fullName, t: 'array', src: [hName+"내과", hName+"소아과", "사랑"+type, "행복"+type] },
                    { q: "점심시간은 언제 시작하나요?", a: lunchStart, t: 'time' },
                    { q: "토요일은 몇 시까지 진료하나요?", a: satClose, t: 'time' },
                    { q: "평일 진료가 끝나는 시간은?", a: closeTime, t: 'time' },
                    { q: "병원이 쉬는 날(휴진)은?", a: closedDay, t: 'array', src: ["토요일", "월요일", "금요일"] },
                    { q: "진료를 시작하는 시간은?", a: openTime, t: 'time' }
                ];
                const q = getRand(qPool);
                options = getDistractors(q.a, q.t, q.src);
                answer = q.a; qList = q.q;
        ...
        // [3-2] 안내문 독해 (심화) - Code 2 generateReadingQuizHard() 이식
        // Code 2의 모든 서술형/심화 시나리오(0~9) 포함
        function generateReadingQuizHard() {
            const makeDate = () => `${getRandomInt(3, 11)}월 ${getRandomInt(1, 28)}일`;
            ...
            const DB = {
                names: ["김철수", "윤지호", "박민준", "최병용", "김민지", "허찬", "박지훈", "장영재", "이은주", "윤준영"],
                places_play: ["우리 집 거실", "방방 놀이터", "학교 앞 분식집", "동네 치킨집"],
                places_public: ["시민공원 분수대", "중앙도서관", "국립과학관", "시립미술관"],
                foods: ["콤비네이션 피자", "순살 양념 치킨", "떡볶이와 튀김", "햄버거 세트"],
                items: ["얼음물", "모자", "돗자리", "우산", "장갑", "마스크"],
            ...
            if (type === 0) {
                ...
                content = `
                    <div>
                        안녕, 친구들아! 나 <strong>${host}</strong>(이)야.<br>
                        드디어 기다리던 내 생일이 다가왔어! 그래서 이번에 너희랑 같이 맛있는 것도 먹고 신나게 놀고 싶어서 초대장을 보내.<br>
                        날짜는 <strong>${date}</strong>이고, 시간은 <strong>${timeStr}</strong>에 만나기로 하자.<br><br>
                        장소는 우리끼리 놀기 딱 좋은 <strong>${place}</strong>(으)로 정했어.<br>
                        내가 용돈을 모아서 너희가 좋아하는 <strong>${food}</strong>(을)를 많이 시켜줄게.<br>
                        그리고 정말 중요한 건데, <strong>선물은 진짜 안 가져와도 돼.</strong> 그냥 몸만 와서 축하해 줘! 꼭 와야 해!
                    </div>`;
                
                qPool = [
                    { q: "편지를 쓴 주인공(생일인 사람)은?", a: host, t: 'array', src: DB.names },
                    { q: "친구들이 만나는 날짜는?", a: date, t: 'date' }, 
                    { q: "약속 시간은 언제인가요?", a: timeStr, t: 'time' },
                    { q: "파티를 하기로 한 장소는?", a: place, t: 'array', src: DB.places_play },
                    { q: "주인공이 준비한 음식은?", a: food, t: 'array', src: DB.foods },
                    { q: "주인공이 친구들에게 부탁한 내용은?", a: "선물 가져오지 않기", t: 'array', src: ["선물 꼭 사오기", "선물 가져오지 않기", "회비 걷기", "동생 데려오기"] }
                ];
            }
            ...
            // --- 시나리오 1: 아파트 안내방송 ---
                ...
                const reason = getRand(["정기 안전 점검", "노후 부품 교체", "승강기 바닥 청소", "비상 벨 점검"]);
                
                title = "📢 승강기 운행 정지 안내";
                content = `
                    <div>
                        [관리사무소] 주민 여러분 안녕하십니까.<br>
                        입주민 여러분의 안전을 위해 오는 <strong>${date}</strong>에 승강기 <strong>${reason}</strong>을 실시할 예정입니다.<br><br>
                        작업 시간은 <strong>${startT}</strong>부터 <strong>${endT}</strong>까지이며, 약 <strong>${duration}시간</strong> 동안 진행됩니다.<br>
                        이 시간 동안은 승강기 운행이 완전히 멈추게 되니, 외출 계획이 있으신 분들은 미리 준비해 주시기 바랍니다.<br>
                        불편하시더라도 안전을 위해 <strong>비상 계단</strong>을 이용해 주시면 감사하겠습니다.
                    </div>`;

                qPool = [
                    { q: "점검을 실시하는 날짜는?", a: date, t: 'date' },
                    { q: "점검이 끝나는 시간은?", a: endT, t: 'time' },
                    { q: "승강기 점검을 하는 이유는?", a: reason, t: 'array', src: ["정기 안전 점검", "노후 부품 교체", "대청소", "페인트 칠"] },
                    { q: "운행 중단 시 이용해야 할 수단은?", a: "비상 계단", t: 'array', src: ["화물용 승강기", "비상 계단", "옆 동 승강기", "옥상 사다리"] },
                    { q: "총 공사(점검) 시간은 몇 시간인가요?", a: duration+"시간", t: 'text_num' }
                ];
            }

            // --- 시나리오 2: 학교 급식 알림 ---
            else if (type === 2) {
                const mainList = ["소고기 불고기덮밥", "치킨마요 덮밥", "김치볶음밥", "잔치국수"];
                const soupList = ["들깨 미역국", "얼큰한 콩나물국", "시금치 된장국", "어묵탕"];
                const dessertList = ["딸기 요거트", "조각 사과", "커스터드 푸딩", "아이스크림"];
                ...
                content = `
                    <div>
                        [학교 알림] 학부모님, 가정에 늘 행복이 가득하시길 바랍니다.<br>
                        우리 학교 급식실에서는 학생들의 건강을 위해 매일 신선한 재료로 정성을 다해 음식을 만들고 있습니다.<br>
                        돌아오는 <strong>${day}</strong>의 점심 식단을 미리 안내해 드립니다.<br><br>
                        이날의 주메뉴는 아이들이 좋아하는 <strong>${main}</strong>이며, 국으로는 따뜻한 <strong>${soup}</strong>이(가) 준비됩니다.<br>
                        식사 후에는 입가심을 위해 달콤한 <strong>${dessert}</strong>까지 제공될 예정이니, 알레르기가 있는 학생은 미리 담임 선생님께 말씀해 주세요.
                    </div>`;

                qPool = [
                    { q: "급식이 제공되는 요일은?", a: day, t: 'array', src: DB.weekdays },
                    { q: "이날의 메인 요리(주메뉴)는?", a: main, t: 'array', src: mainList },
                    { q: "함께 나오는 국의 종류는?", a: soup, t: 'array', src: soupList },
                    { q: "식사 후에 먹는 후식은?", a: dessert, t: 'array', src: dessertList }
                ];
            }

            // --- 시나리오 3: 도서관 연체 문자 ---
            else if (type === 3) {
                const bookList = ["해리포터와 마법사의 돌", "Why? 우주와 로켓", "흔한남매의 일상", "그리스로마신화"];
                ...
                content = `
                    <div>
                        [도서관 알림톡] 회원님께서 빌려가신 책 <strong>『${book}』</strong>의 반납 기간이 지났습니다.<br>
                        원래대로라면 <strong>${retDate}</strong>까지 반납해주셨어야 하는데 아직 확인되지 않고 있습니다.<br>
                        지금 이 책을 보고 싶어서 기다리는 다른 친구들이 많이 있습니다.<br><br>
                        오늘까지 계산된 연체료는 <strong>${fee}원</strong>입니다.<br>
                        내일까지 꼭 도서관 <strong>1층 안내데스크</strong>로 오셔서 책을 돌려주시고 연체료도 내주시기 바랍니다.
                    </div>`;

                qPool = [
                    { q: "돌려주지 않은 책의 제목은?", a: book, t: 'array', src: bookList },
                    { q: "원래 책을 반납했어야 할 날짜는?", a: retDate, t: 'date' },
                    { q: "지금까지 밀린 연체료는 얼마?", a: fee+"원", t: 'text_num' },
                    { q: "책을 반납해야 하는 장소는?", a: "1층 안내데스크", t: 'array', src: ["1층 안내데스크", "2층 열람실", "경비실", "교무실"] }
                ];
            }

            // --- 시나리오 4: 방과후 수업 변경 ---
            else if (type === 4) {
                const classList = ["축구 교실", "미술반", "로봇 과학", "요리 교실", "음악 줄넘기"];
                const placeList = ["강당", "음악실", "과학실", "가사실", "운동장"];
                ...
                content = `
                    <div>
                        안녕하세요. <strong>${className}</strong> 수업을 가르치는 <strong>${teacher}</strong> 선생님입니다.<br>
                        오늘 우리가 수업하려던 교실 천장에 갑자기 물이 새는 바람에 긴급 공사를 하게 되었습니다.<br><br>
                        그래서 오늘 하루만 수업 장소를 바꿔야 할 것 같아요.<br>
                        학생들은 평소 가던 교실로 가지 말고 <strong>${newRoom}</strong>(으)로 바로 와주세요.<br>
                        수업 시간은 바뀌지 않고 그대로니까 늦지 않게 와주세요. 불편을 줘서 미안해요!
                    </div>`;

                qPool = [
                    { q: "어떤 수업에 대한 안내인가요?", a: className, t: 'array', src: classList },
                    { q: "선생님의 이름은?", a: teacher, t: 'array', src: DB.names },
                    { q: "변경된 수업 장소는 어디인가요?", a: newRoom, t: 'array', src: placeList }
                ];
            }
            ...
            // --- 시나리오 5: 일기 예보 ---
                ...
                const item = weather === "비" ? "우산" : (weather === "눈" ? "장갑" : (weather === "맑음" ? "모자" : "겉옷"));
                const humidity = getRandomInt(40, 80);
                
                title = "🌤 오늘의 날씨";
                content = `
                    <div>
                        여러분 안녕하십니까, 기상 캐스터입니다.<br>
                        오늘 우리 지역의 날씨를 알려드리겠습니다.<br>
                        현재 구름의 영향으로 오늘 하루는 <strong>${weather}</strong> 상태가 계속될 것으로 보입니다.<br>
                        습도는 ${humidity}% 정도이며, 낮 최고 기온은 <strong>${temp}도</strong>까지 올라가겠습니다.<br><br>
                        아침저녁으로 기온 차이가 크니 외출하실 때는 <strong>${item}</strong>을(를) 챙기시는 게 좋겠습니다.<br>
                        건강 관리에 유의하시기 바랍니다.
                    </div>`;

                qPool = [
                    { q: "오늘의 날씨 상태는?", a: weather, t: 'array', src: weatherList },
                    { q: "낮 최고 기온은 몇 도인가요?", a: temp+"도", t: 'text_num' },
                    { q: "외출 시 챙겨야 할 물건은?", a: item, t: 'array', src: DB.items }
                ];
            }
            ...
            // --- 시나리오 6: 요리 레시피 ---
                ...
                const addList = ["대파", "계란", "치즈", "떡", "만두"];
                const add = getRand(addList); 

                title = "🍜 맛있는 라면 끓이기";
                content = `
                    <div>
                        집에서도 분식집처럼 맛있는 라면을 끓이는 방법을 알려드릴게요.<br>
                        가장 중요한 건 물의 양인데요, 냄비에 물 <strong>${water}ml</strong>를 정확하게 넣고 센 불로 끓여주세요.<br><br>
                        물이 보글보글 끓으면 면과 스프를 넣고 <strong>${time}분</strong> 동안 더 끓여야 면이 쫄깃해집니다.<br>
                        마지막으로 불을 끄기 직전에 <strong>${add}</strong>을(를) 넣으면 국물 맛이 훨씬 좋아진답니다.<br>
                        오늘 점심으로 따뜻한 라면 한 그릇 어떠세요?
                    </div>`;

                qPool = [
                    { q: "물의 양은 얼마를 넣어야 하나요?", a: water+"ml", t: 'text_num' },
                    { q: "면을 넣고 몇 분간 끓여야 하나요?", a: time+"분", t: 'text_num' },
                    { q: "마지막에 넣는 재료는?", a: add, t: 'array', src: addList }
                ];
            }

            // --- 시나리오 7: 강아지 찾기 ---
            else if (type === 7) {
                const dogList = ["뽀삐", "초코", "두부", "하늘이"];
                const breedList = ["말티즈", "푸들", "시바견", "리트리버"];
                const colorList = ["하얀색", "갈색", "검은색", "누런색"];
                ...
                content = `
                    <div>
                        소중한 저희 집 강아지를 잃어버려서 찾고 있습니다.<br>
                        이름은 <strong>${dogName}</strong>이고, <strong>${breed}</strong>이며 털 색깔은 <strong>${color}</strong>입니다.<br><br> 
                        지난주에 <strong>${location}</strong> 근처에서 산책을 하다가 그만 놓치고 말았습니다.<br>
                        사람을 아주 잘 따르고 순한 성격입니다.<br>
                        보호하고 계시거나 비슷한 강아지를 보신 분은 <strong>${contact}</strong>(으)로 꼭 연락 부탁드립니다.<br>
                        사례하겠습니다.
                    </div>`;

                qPool = [
                    { q: "잃어버린 강아지의 이름은?", a: dogName, t: 'array', src: dogList },
                    { q: "강아지의 종류(견종)는?", a: breed, t: 'array', src: breedList },
                    { q: "강아지의 털 색깔은?", a: color, t: 'array', src: colorList },
                    { q: "주인의 연락처 뒷자리는?", a: contact.split('-')[2], t: 'text_num' }
                ];
            }

            // --- 시나리오 8: 박물관 설명 ---
            else if (type === 8) {
                const relicList = ["금관", "청자", "동검", "토기"];
                const eraList = ["신라", "고려", "조선", "고구려"];
                const usageList = ["제사", "전쟁", "저장", "장식"];
                ...
                content = `
                    <div>
                        관람객 여러분, 이 유물을 봐주세요. 우리 역사에서 아주 중요한 보물입니다.<br>
                        이것은 옛날 <strong>${era}</strong> 시대에 만들어진 <strong>${relic}</strong>입니다.<br>
                        모양이 아주 독특하고 아름답지 않나요?<br><br>
                        역사학자들의 연구에 따르면, 옛날 사람들은 이것을 주로 <strong>${usage}</strong>을(를) 할 때 사용했다고 합니다.<br>
                        오래된 물건이라 깨지기 쉬우니 눈으로만 봐주시고 절대 손으로 만지지 마세요.
                    </div>`;

                qPool = [
                    { q: "어느 시대의 유물인가요?", a: era, t: 'array', src: eraList },
                    { q: "전시된 유물의 이름은?", a: relic, t: 'array', src: relicList },
                    { q: "옛날에 어떤 용도로 쓰였나요?", a: usage, t: 'array', src: usageList }
                ];
            }
            
            // --- 시나리오 9: 체험학습 안내 (가정통신문 상세) ---
            else {
                const destList = ["놀이공원", "동물원", "식물원", "역사 박물관", "민속촌"];
                ...
                content = `
                    <div>
                        학부모님 안녕하십니까? 우리 학교에서는 학생들의 다양한 경험을 위해<br>
                        이번 주 <strong>${day}</strong>에 <strong>${dest}</strong>(으)로 체험학습을 떠나기로 했습니다.<br><br>
                        출발 당일 학생들은 늦어도 <strong>${time}</strong>까지 학교 운동장으로 모여야 합니다.<br>
                        활동하기 편한 운동화를 신고 오게 해주시고, 개인 준비물로는 <strong>${item}</strong>과 맛있는 점심 도시락을 챙겨주세요.<br>
                        ${busNum}호차 버스를 타고 안전하고 즐겁게 다녀오겠습니다.
                    </div>`;

                qPool = [
                    { q: "체험학습을 가는 요일은?", a: day, t: 'array', src: DB.weekdays },
                    { q: "어디로 견학을 가나요?", a: dest, t: 'array', src: destList },
                    { q: "몇 시까지 운동장으로 모여야 하나요?", a: time, t: 'time' },
                    { q: "꼭 챙겨야 할 준비물은?", a: item, t: 'array', src: DB.items }
                ];
            }
            ...
            if (q.t === 'date') {
                ...
                let oSet = new Set([answer]);
                if (answer.includes('시') && (answer.includes('오전') || answer.includes('오후'))) {
                    let hour = parseInt(answer.replace(/[^0-9]/g, ''));
                    const isAm = answer.includes('오전');
                    while(oSet.size < 4) {
                        ...
                        let wrongStr = (wrongAm ? '오전 ' : '오후 ') + wrongHour + '시';
                        if (wrongStr !== answer) oSet.add(wrongStr);
                        if (oSet.size < 4) oSet.add(`오전 ${getRandomInt(1,12)}시`);
                    }
                } 
                     ...
                     while(oSet.size < 4) oSet.add(getRandomInt(1, 12) + "시");
                }
                options = shuffleArray(Array.from(oSet));
            }
            else {
            ...
            return {
                problem: q.q, // 텍스트만 리턴 (화면 표시는 nextLearningProblem에서 함)
                answer: answer,
                options: options,
                readingData: { title: title, content: content } 
            };
        ...
        // [3-3] 팝업에 독해 내용 표시 (공통 함수)
        function renderReadingText(pid, data) {
            // 이 함수는 1번 코드 구조에서는 'data-modal'을 띄울 때 사용됩니다.
   ...
   // ==========================================
        // [4] 문제 생성 로직: 수학 & 기초 학습 (Code 2 완벽 이식)
        // ==========================================

        // [4-1] 돈 계산 퀴즈 (Code 2: 7가지 유형 - 거스름돈, 총액, 잔액, 구매가능, 합계, 더치페이, 저축)
        function generateMoneyQuiz(depth=0) {
            if(depth > 10) return { problem: "오류", answer: 0, options: ["0","1","2","3"] }; 
            
            const items = ['우유', '과자', '껌', '공책', '연필', '지우개', '음료수', '아이스크림', '샌드위치'];
            const prices = [500, 800, 300, 1000, 400, 200, 1200, 1500, 2500];
            
            // 0~6까지 유형 확장 (5: 더치페이, 6: 저축하기)
   ...
   // [유형 0] 거스름돈 계산
            if (type === 0) { 
                ...
                // [수정] 서술형 변경
                p = `${held}원을 내고 ${price}원짜리 ${item} 1개를 샀습니다. 거스름돈은 얼마인가요?`; 
                a = held - price;
            ...
            // [유형 1] 총 가격 (단순 곱셈)
                ...
                // [수정] 서술형 변경
                p = `${price}원짜리 ${item} ${cnt}개를 사면 모두 얼마인가요?`; 
                a = price * cnt;
            ...
            // [유형 2] 여러 개 사고 남은 돈
                ...
                // [수정] 서술형 변경
                p = `${held}원을 가지고 ${price}원짜리 ${item} ${cnt}개를 샀습니다. 남은 돈은 얼마인가요?`; 
                a = held - cost;
                o.add(a); o.add(a+100); o.add(a-100); o.add(held - price);
            } 
            // [유형 3] 구매 가능 여부 (Yes/No)
            else if (type === 3) { 
                const idx = getRandomInt(0, items.length-1);
                const item = items[idx];
                const price = prices[idx];
                ...
                // [수정] 서술형 변경
                p = `${myMoney}원으로 ${price}원짜리 ${item} ${cnt}개를 살 수 있을까요?`;
                a = isPossible ? "살 수 있다" : "살 수 없다";
                o.add("살 수 있다"); o.add("살 수 없다"); o.add("모르겠다"); o.add("글쎄요");
            } 
            // [유형 4] 두 물건 합계
            else if (type === 4) { 
                const idx = getRandomInt(0, items.length-1);
                ...
                // [수정] 서술형 변경
                p = `${prices[idx]}원짜리 ${items[idx]} 1개와 ${prices[idx2]}원짜리 ${items[idx2]} 1개는 모두 얼마인가요?`;
                a = prices[idx] + prices[idx2];
                o.add(a); o.add(a+100); o.add(a-100); o.add(Math.abs(prices[idx]-prices[idx2]));
            }
            // [유형 5] 더치페이 (나누어 내기)
            else if (type === 5) {
                const people = getRandomInt(2, 4); // 2~4명
                const perPerson = getRandomInt(2, 9) * 1000; // 1인당 2000~9000원
                const totalBill = people * perPerson;

                // [수정] 서술형 변경
                p = `음식값 ${totalBill}원을 ${people}명이 똑같이 나누어 내면 한 사람은 얼마를 내야 하나요?`;
                a = perPerson;
                
                o.add(a); 
                o.add(a + 1000); 
                o.add(a - 1000); 
                o.add(Math.floor(totalBill / (people + 1))); // 인원수 착각 오답
            }
            // [유형 6] 저축 목표
            else if (type === 6) {
                const weeks = getRandomInt(3, 8); // 3~8주
                const saving = getRandomInt(2, 5) * 1000; // 매주 2000~5000원 저축
                const target = weeks * saving;
                
                // [수정] 서술형 변경
                p = `${target}원짜리 장난감을 사려고 매주 ${saving}원씩 모읍니다. 모두 모으려면 몇 주가 걸릴까요?`;
                a = weeks;
                unit = "주";
            ...
            if (typeof a === 'number') {
                ...
                if (type === 6) {
                    ...
                    return { 
                        problem: p, 
                        answer: a + "원", // Code 2는 숫자만 관리했으나 여기선 "원" 붙여줌
                        options: shuffleArray(safeO).map(opt => opt + "원")
                    };
                }
            } else {
                // "살 수 있다/없다" 같은 문자열 정답인 경우
                return { 
                    problem: p, 
                    answer: a, 
        ...
        function generateUnitProblem() {
            ...
            const templates = [
                ...
                { large: '분', small: '초', factor: 60, type: 'time' },
                { large: '시간', small: '분', factor: 60, type: 'time' },
                { large: '일', small: '시간', factor: 24, type: 'day' },
                { large: '년', small: '일', factor: 365, type: 'year' }
            ];
            ...
            const direction = getRandomInt(0, 1); // 0: 큰->작, 1: 작->큰
            let p, ans;
            let o = new Set();
            ...
            if (direction === 0) {
                // 예: 2m -> 200cm
                ans = val * t.factor; 
                p = `<span class="text-3xl font-bold text-blue-600">${val}${t.large}</span><br><span class="text-sm text-gray-500">는 몇 ${t.small}일까요?</span>`;
                o.add(ans);
                ...
                ans = val; 
                p = `<span class="text-3xl font-bold text-green-600">${questionVal}${t.small}</span><br><span class="text-sm text-gray-500">은 몇 ${t.large}일까요?</span>`;
                o.add(ans);
        ...
        function generateMathQuiz(op, min, max) {
            ...
            // 가로/세로 모드 (기본은 세로, 필요시 변경 가능하도록 설계)
            let problemHtml = '';
            // 여기서는 1번 코드 스타일인 세로셈을 기본으로 적용하되, Code 2의 HTML 구조를 따름
            problemHtml = `
        ...
        function generateNumberSystemQuiz(level) {
            ...
            let p, a, o = new Set();
            if (type===0) { 
                p = `<span class="text-sm text-gray-400">숫자</span><br>${targetNum}<br><span class="text-sm text-gray-400">우리말로?</span>`; 
                ...
                p = `<span class="text-sm text-gray-400">우리말</span><br>${v}<br><span class="text-sm text-gray-400">숫자로?</span>`; 
                a = targetNum; 
                o.add(a); 
        ...
        function generateAlphabetQuiz(mode) {
            ...
            const alphaReads = ['에이','비','씨','디','이','에프','지','에이치','아이','제이','케이','엘','엠','엔','오','피','큐','알','에스','티','유','브이','더블유','엑스','와이','지(제트)'];
            const idx = getRandomInt(0, 25); const char = alphabets[idx];
            let p, a, o = new Set();
            if(mode==='order') { p=`${char} 다음은?`; a=alphabets[idx+1]||'A'; o.add(a); while(o.size<4) o.add(alphabets[getRandomInt(0,25)]); }
            else if(mode==='case') { p=`${char}의 소문자는?`; a=char.toLowerCase(); o.add(a); while(o.size<4) o.add(alphabets[getRandomInt(0,25)].toLowerCase()); }
            else { p=`${char} 읽는 법?`; a=alphaReads[idx]; o.add(a); while(o.size<4) o.add(alphaReads[getRandomInt(0,25)]); }
            return { problem: `<span class="text-5xl font-bold text-blue-500">${p}</span>`, answer: a, options: shuffleArray(Array.from(o)) };
        }
   // ==========================================
        // [5] 문제 생성 로직: 달력, 일과표, 시간, 문장제 (Code 2 완벽 이식)
        // ==========================================

  // [달력 전용 상수 선언 - 인덱스 불일치 해결을 위해 독립 정의]
const CAL_DAYS = ['일', '월', '화', '수', '목', '금', '토'];

// [5-1] 달력 데이터 생성 (토요일 로직 수정)
function createRandomMonth() {
    const lastDate = [30, 31][getRandomInt(0, 1)];
    const startDayIndex = getRandomInt(0, 6); // 0:일요일 ~ 6:토요일
    const holidays = new Set();
    
    // 일요일은 자동으로 holidays에 추가하지 않고 CSS 클래스로 처리 (CSS 충돌 방지)
    // 랜덤 평일 휴일 추가 (1~3일 정도)
    while(holidays.size < getRandomInt(1, 3)) {
        const d = getRandomInt(1, lastDate);
        const dayIdx = (startDayIndex + d - 1) % 7;
        // 토, 일요일이 아닐 때만 휴일(빨간날) 추가
...
// [5-1-B] 달력 HTML 그리기 (CSS 클래스 매핑 수정)
function getCalendarHTML(md) {
    const { lastDate, startDayIndex, holidays } = md;
    let html = '<div class="calendar-grid-container">';
    ...
    for(let d=1; d<=lastDate; d++) {
        let cls = ''; // 기본 클래스 없음
        const dayIdx = (startDayIndex + d - 1) % 7; // 0(일) ~ 6(토)
...
function generateCalendarQuestion() {
    ...
    if (type === 0) { 
        ...
        p = `<span class="text-blue-600 font-bold">${t}일</span>은 무슨 요일인가요?`; 
        a = CAL_DAYS[dayIdx] + '요일';
        
        o.add(a); 
        while(o.size < 4) o.add(CAL_DAYS[getRandomInt(0, 6)] + '요일');
    } 
    // 유형 1: N일 뒤 요일 찾기
    else if (type === 1) { 
        const after = getRandomInt(2, 10);
        // 날짜가 달을 넘어가지 않도록 조정하거나, 단순히 요일만 계산
        // 여기서는 요일만 물어보므로 단순 계산
        const futureDate = today + after;
        // 미래 날짜가 이번달을 넘어가도 요일 규칙은 같음
        const futureDayIdx = getDayIdx(futureDate);
        
        p = `오늘이 <span class="text-blue-600 font-bold">${today}일</span>이라면<br>${after}일 뒤는 무슨 요일?`; 
        a = CAL_DAYS[futureDayIdx] + '요일';
        
        o.add(a); 
        while(o.size < 4) o.add(CAL_DAYS[getRandomInt(0, 6)] + '요일');
    } 
    // 유형 2: 특정 요일 개수 세기 (예: 일요일이 몇 번?)
    else if (type === 2) { 
        ...
        p = `<span class="text-green-600 font-bold">${targetName}요일</span>은<br>모두 몇 번 있나요?`; 
        a = cnt + "번";
        
        o.add(a); o.add("3번"); o.add("4번"); o.add("5번");
    } 
    // 유형 3: 빨간 날(휴일+일요일) 개수 세기
    else if (type === 3) { 
        let cnt = 0;
        for(let d=1; d<=lastDate; d++) {
            // 일요일(0)이거나 holidays에 포함된 경우
            if(getDayIdx(d) === 0 || holidays.has(d)) cnt++;
        }
        
        p = `달력의 <span class="text-red-500 font-bold">빨간 날</span>은<br>모두 며칠인가요?`; 
        a = cnt + "일";
        
        o.add(a); o.add((cnt+1)+"일"); o.add((cnt-1)+"일"); o.add((cnt+2)+"일");
    } 
    // 유형 4: N번째 X요일은 며칠?
    else if (type === 4) { 
        const targetIdx = getRandomInt(0, 6);
        ...
        p = `<span class="text-purple-600 font-bold">${nth}번째 ${targetName}요일</span>은<br>며칠인가요?`; 
        a = targetDate + "일";
        
        o.add(a); 
        o.add((targetDate+7 > lastDate ? targetDate-7 : targetDate+7) + "일");
        o.add((targetDate+1) + "일");
        o.add((targetDate-1) + "일");
    } 
    // 유형 5: 날짜 간격 계산
    else { 
        ...
        p = `${d1}일부터 ${d2}일까지는<br>모두 며칠일까요?<br><span class="text-sm text-gray-400">(시작일 포함)</span>`; 
        a = (d2 - d1 + 1) + "일";
        
        const wrong1 = (d2 - d1);
        o.add(a); o.add(wrong1 + "일"); o.add((wrong1+2) + "일"); o.add((wrong1-1) + "일");
    }
    
    // 보기 4개로 맞추기
    const finalOptions = Array.from(o);
    while(finalOptions.length < 4) {
        const r = getRandomInt(1, 31) + "일"; // 대충 아무거나 추가
    ...
    return { 
        ...
        monthData: md // 보기 버튼 클릭 시 팝업에 띄울 데이터
    };
}

        // [5-3] 일과표 분석 퀴즈 (Code 2: 6가지 유형 - 직전/직후/시간차 등)
        function generateScheduleQuiz(depth=0) {
            ...
            if(type===0) { 
                ...
                if(tIdx < items.length-1) {
                    ...
                    p = `${timeStr}에는<br>무엇을 하고 있나요?`; a = tItem.a;
                    o.add(a); while(o.size<4) o.add(items[getRandomInt(0, items.length-1)].a);
                } else {
                ...
                if(tIdx < items.length-1) {
                    ...
                    p = `"${item1.a}"부터<br>"${item2.a}" 전까지<br>시간은 얼마인가요?`;
                    const h1 = parseInt(item1.t.split(':')[0]);
                    const h2 = parseInt(item2.t.split(':')[0]);
                    let diff = h2 - h1; 
                    if (diff < 0) diff += 24;
                    a = diff + "시간";
                    o.add(a); o.add((diff+1)+"시간"); o.add((diff-1)+"시간"); o.add((diff*60)+"분");
                } else {
                    return generateScheduleQuiz(depth+1);
                }
            } else if(type===5) { 
                const targetItem = items.find(i => i.a.includes('퇴근') || i.a.includes('하교') || i.a.includes('집에가기'));
                if(targetItem) {
                    ...
                    const eventName = (targetItem.a.includes('퇴근') || targetItem.a.includes('집에가기')) ? "야근" : "보충수업";
                    p = `<span class="text-blue-600 font-bold">${addedTime}시간 ${eventName}</span>을 하면<br>몇 시에 집에 가게 될까요?`;
                    a = ansTimeStr;
            ...
            return { 
                ...
                answer: a, 
                options: shuffleArray(Array.from(o)), 
                scheduleData: d // Code 1 UI에서 일과표 그리기에 사용
            };
        }

        function generateScheduleQuizSafeFallback() {
             const d = SCHEDULE_DATA['STUDENT'];
             return { problem: "학교 갈 시간은?", answer: "08:00", options: ["07:00","08:00","09:00","10:00"], scheduleData: d };
        }

   // [5-X] 시계 보기 퀴즈 생성 함수 (누락된 로직 추가)
        function generateClockQuiz(level) {
            ...
            while(o.size < 4 && loopGuard < 50) {
                ...
                if(wh !== h) o.add(`${wh}시 ${m}분`);
                
                // 오답 패턴 2: 분만 다르게
                let wm;
                if(level === 'easy') wm = m === 0 ? 30 : 0;
                else wm = getRandomInt(0, 59);
                if(wm !== m) o.add(`${h}시 ${wm}분`);
                
                // 오답 패턴 3: 시와 분을 바꿈 (예: 3시 6분 -> 6시 3분)
                if(m >= 1 && m <= 12) o.add(`${m}시 ${h * 5}분`); // 바늘 위치 착각 유도
            }
            
            // 부족한 보기 채우기
            while(o.size < 4) {
                o.add(`${getRandomInt(1,12)}시 ${getRandomInt(0,5)*10}분`);
            }

            // 시계 그리기는 nextQuiz 함수에서 q.h, q.m 데이터를 받아 처리함
return { 
                problem: "시계가 나타내는 시각은?", 
                answer: answerStr, 
        ...
        // [5-4] 시간 계산 (Code 2: Easy/Hard 난이도 분기 로직)
        function generateTimeProblem(level) {
            ...
            const durationStr = `${dh > 0 ? dh+'시간 ' : ''}${dm > 0 ? dm+'분' : ''}`;
            const problemHtml = `${startTimeStr}<br><span class="text-blue-600 font-bold">${durationStr} ${isAfter ? '후' : '전'}</span>는?`;
        ...
        // [5-5] 문장제 문제 (Code 2: 40개 시나리오 전체 포함)
        function generateWordProblem(type) {
            // 1. 덧셈 시나리오
            const scenariosAdd = [
                { t: "버스에 {0}명이 타고 있었습니다.<br>정류장에서 {1}명이 더 탔다면<br>모두 몇 명일까요?", unit: "명" },
                { t: "냉장고에 사과가 {0}개,<br>식탁에 {1}개 있습니다.<br>사과는 모두 몇 개일까요?", unit: "개" },
                { t: "철수는 딱지를 {0}장 가지고 있고,<br>영희는 {1}장 가지고 있습니다.<br>두 사람의 딱지는 모두 몇 장?", unit: "장" },
                { t: "아침에 물을 {0}잔 마시고,<br>저녁에 {1}잔 마셨습니다.<br>마신 물은 모두 몇 잔일까요?", unit: "잔" },
                { t: "필통에 연필이 {0}자루,<br>책상 위에 {1}자루 있습니다.<br>연필은 모두 몇 자루일까요?", unit: "자루" },
                { t: "놀이터에 비둘기가 {0}마리 있고,<br>{1}마리가 더 날아왔습니다.<br>비둘기는 모두 몇 마리일까요?", unit: "마리" },
                { t: "어제 동화책을 {0}권 읽고,<br>오늘 {1}권 더 읽었습니다.<br>읽은 책은 모두 몇 권일까요?", unit: "권" },
                { t: "주차장에 차가 {0}대 있었는데,<br>{1}대가 더 들어왔습니다.<br>차는 모두 몇 대일까요?", unit: "대" },
                { t: "지우개 {0}개와<br>연필 {1}개를 샀습니다.<br>학용품은 모두 몇 개일까요?", unit: "개" },
                { t: "빨간 풍선 {0}개와<br>파란 풍선 {1}개를 불었습니다.<br>풍선은 모두 몇 개일까요?", unit: "개" }
            ];
            // 2. 뺄셈 시나리오
            const scenariosSub = [
                { t: "쿠키가 {0}개 있었습니다.<br>친구와 {1}개를 나눠 먹었다면<br>남은 쿠키는 몇 개일까요?", unit: "개" },
                { t: "버스에 {0}명이 타고 있었습니다.<br>이번 역에서 {1}명이 내렸다면<br>몇 명이 남았을까요?", unit: "명" },
                { t: "달걀 {0}개 중에서<br>{1}개가 깨졌습니다.<br>남은 달걀은 몇 개일까요?", unit: "개" },
                { t: "나무에 참새 {0}마리가 앉아있다가<br>{1}마리가 날아갔습니다.<br>남은 참새는 몇 마리일까요?", unit: "마리" },
                { t: "용돈을 {0}천 원 받아서<br>{1}천 원을 썼습니다.<br>얼마가 남았을까요? (단위: 천 원)", unit: "" },
                { t: "책이 {0}권 꽂혀있었는데<br>{1}권을 빌려 갔습니다.<br>남은 책은 몇 권일까요?", unit: "권" },
                { t: "우유가 {0}개 있었습니다.<br>{1}개를 마셨다면<br>몇 개가 남았을까요?", unit: "개" },
                { t: "주차장에 차가 {0}대 있었는데,<br>{1}대가 나갔습니다.<br>남은 차는 몇 대일까요?", unit: "대" },
                { t: "색종이 {0}장 중에서<br>{1}장을 사용했습니다.<br>남은 색종이는 몇 장일까요?", unit: "장" },
                { t: "사탕 {0}개 중<br>{1}개를 동생에게 주었습니다.<br>내가 가진 사탕은 몇 개일까요?", unit: "개" }
            ];
            // 3. 곱셈 시나리오
            const scenariosMul = [
                { t: "거미의 다리는 {0}개입니다.<br>거미 {1}마리의 다리는<br>모두 몇 개일까요?", unit: "개", fixed: 8 },
                { t: "자동차 한 대에 바퀴가 {0}개입니다.<br>자동차 {1}대의 바퀴는<br>모두 몇 개일까요?", unit: "개", fixed: 4 },
                { t: "한 상자에 도넛이 {0}개씩 들어있습니다.<br>{1}상자의 도넛은<br>모두 몇 개일까요?", unit: "개" },
                { t: "학생 한 명에게 공책을 {0}권씩<br>주려고 합니다. {1}명에게 주려면<br>공책은 몇 권 필요할까요?", unit: "권" },
                { t: "일주일은 {0}일입니다.<br>{1}주일은 모두 며칠일까요?", unit: "일", fixed: 7 },
                { t: "사탕이 한 봉지에 {0}개씩 있습니다.<br>{1}봉지에 들어있는 사탕은<br>모두 몇 개일까요?", unit: "개" },
                { t: "딱정벌레의 다리는 {0}개입니다.<br>딱정벌레 {1}마리의 다리는<br>모두 몇 개일까요?", unit: "개", fixed: 6 },
                { t: "하루에 줄넘기를 {0}번씩 합니다.<br>{1}일 동안 줄넘기를<br>모두 몇 번 했을까요?", unit: "번" },
                { t: "한 묶음에 꽃이 {0}송이 있습니다.<br>{1}묶음의 꽃은<br>모두 몇 송이일까요?", unit: "송이" },
                { t: "500원짜리 동전을<br>{1}개 가지고 있습니다.<br>모두 얼마일까요?", unit: "원", fixed: 500 }
            ];
            // 4. 나눗셈 시나리오
            const scenariosDiv = [
                { t: "사탕 {0}개를<br>친구 {1}명에게 똑같이 나누어 주면<br>한 명당 몇 개씩 가질까요?", unit: "개" },
                { t: "연필 {0}자루를<br>한 명에게 {1}자루씩 주면<br>몇 명에게 줄 수 있을까요?", unit: "명" },
                { t: "귤 {0}개를<br>한 바구니에 {1}개씩 담으려 합니다.<br>바구니는 몇 개 필요할까요?", unit: "개" },
                { t: "학생 {0}명이<br>한 모둠에 {1}명씩 짝을 지으면<br>몇 모둠이 생길까요?", unit: "모둠" },
                { t: "책 {0}권을<br>책꽂이 {1}칸에 똑같이 나누어 꽂으면<br>한 칸에 몇 권씩 꽂힐까요?", unit: "권" },
                { t: "길이가 {0}cm인 리본을<br>{1}명이 똑같이 나누어 가지면<br>한 명이 몇 cm를 가질까요?", unit: "cm" },
                { t: "우유 {0}개를<br>{1}일 동안 매일 똑같이 마신다면<br>하루에 몇 개씩 마셔야 할까요?", unit: "개" },
                { t: "장미꽃 {0}송이를<br>꽃병 {1}개에 똑같이 나누어 꽂으려 합니다.<br>한 병에 몇 송이씩 꽂힐까요?", unit: "송이" },
                { t: "만두 {0}개를<br>접시 {1}개에 똑같이 나누어 담으면<br>한 접시에 몇 개씩 담길까요?", unit: "개" },
                { t: "종이학 {0}마리를<br>{1}명이 똑같이 나누어 가졌습니다.<br>한 명이 몇 마리를 가졌을까요?", unit: "마리" }
            ];
   ...
   // ==========================================
        // [6] 비주얼 & 심화 수학 문제 생성 (Code 2 완벽 이식)
        // ==========================================
// [신규] 저울 게임 로직
        function generateInteractiveScale() {
            ...
            return {
                ...
                target: target,
                answer: target, // 정답 확인용
                problem: 'scale_ui' // 렌더링용 플래그
            };
        }
   ...
   function drawInteractiveScale(id, weight) {
    ...
    // [수정] 높이를 400으로 늘려 상단 여백 확보
    cvs.width = 300; cvs.height = 400; 
    const w = cvs.width; const h = cvs.height;
    
    // [수정] bottomY를 h - 50으로 설정하여 아래쪽에도 여유를 둠
    const centerX = w / 2; const bottomY = h - 50;
        ...
        function generateGroupCountingQuiz() {
            ...
            if (type === 0) {
                qText = `<span class="text-blue-600 font-bold">한 묶음에</span> 몇 개씩 있나요?`;
                ansVal = perGroup; unit = "개씩";
            } else if (type === 1) {
                qText = `모두 <span class="text-green-600 font-bold">몇 묶음</span>인가요?`;
                ansVal = groups; unit = "묶음";
            } else if (type === 2) {
                qText = `모두 <span class="text-purple-600 font-bold">몇 개</span>일까요?`;
                ansVal = total; unit = "개";
            } else {
                let seq = [];
                ...
                }).join(`<span class="text-gray-400 font-light mx-1 text-lg">➜</span>`);

                qText = `
                    <div class="text-lg text-gray-500 mb-2 whitespace-nowrap font-bold">빈칸에 알맞은 수는?</div>
        ...
        function generateCompareSizeQuiz(level) {
            ...
            const problemHtml = `
                <div class="flex flex-col items-center justify-center w-full h-[200px] py-1">
                    ...
                    <div class="text-base font-bold text-gray-500 mt-3">알맞은 부등호는?</div>
                </div>
            `;
        ...
        function generateRecycleQuiz() {
            ...
            const categories = ["플라스틱", "종이", "캔", "유리", "비닐", "일반쓰레기", "음식물", "스티로폼"];
            ...
            const problemHtml = `
                <div class="flex flex-col items-center justify-center h-full">
                    <div class="text-sm text-gray-400 font-bold mb-4 tracking-widest uppercase">어디에 버릴까요?</div>
        ...
        function generateFractionBasicQuiz() {
            ...
            // 유형 1: 가분수 -> 대분수
                ...
                questionText = "대분수로 바꾸면?";
                answer = `${whole}와 ${num}/${den}`;
                o.add(answer); o.add(`${whole + 1}와 ${num}/${den}`); o.add(`${whole}와 ${num + 1}/${den}`);
                let wrongNum = (num - 1 > 0) ? num - 1 : num + 2;
                o.add(`${whole}와 ${wrongNum}/${den}`);
            } 
            // 유형 2,3,4: 크기 비교
            else {
            ...
            while(o.size < 4 && safeGuard < 20) {
                 ...
                 } else if (typeof answer === 'string' && answer.includes('와')) {
                     o.add(`${getRandomInt(1,5)}와 ${getRandomInt(1,den-1)}/${den}`);
                 }
                 safeGuard++;
            }
            while(o.size < 4) o.add("오답" + o.size);
            ...
            const problemHtml = `
                <div class="flex flex-col items-center justify-center w-full h-full p-2">
                    <div class="bg-white border-2 border-indigo-100 rounded-2xl py-4 px-6 w-full max-w-[90%] shadow-lg flex flex-col items-center gap-2">
                        <div class="text-indigo-500 text-xs font-bold mb-1 uppercase tracking-wider">네모에 들어갈 가장 큰 수는?</div>
                    ...
                    <div class="text-center mt-3 leading-tight space-y-1">
                        <p class="text-gray-800 font-bold text-sm">${target}<span class="text-gray-400 font-normal">보다</span> <span class="text-red-500">작으면서</span></p>
                        <p class="text-indigo-600 font-bold text-sm">가장 가까운 수</p>
                    </div>
                </div>
            `;
   ...
   // ==========================================
        // [7] 퀴즈 설정 및 연결 (Code 2 difficultySettings 완벽 이식)
        // Code 1의 메뉴 ID와 Code 2의 생성 로직을 매핑하는 설정값입니다.
        // ==========================================
   const QUIZ_CONFIG = {
            // [1단계: 반사신경류 - S:1.2 / A:2.0 / B:3.0]
            ...
            // [2단계: 기초 연산/단어 - S:1.5 / A:2.5 / B:4.0]
            ...
            'div_basic': { type: 'complex_div', hasRem: false, level: 'easy', limits: [2.0, 3.5, 5.0] }, // 구구단 나눗셈

            // [4단계: 고난이도 - S:3.5~ / A:5.5~]
        ...
        // ==========================================
        // [8] 문제 출제 메인 함수 (Dispatcher)
        // ==========================================
        function generateProblem(subject) {
            const config = QUIZ_CONFIG[subject.id];
            
            // 설정이 없으면 기본값 반환
            if (!config) return { problem: "준비 중", answer: "0", options: ["0"] };

            // [국어/영어 계열]
            if (config.type === 'lang') {
                if (config.subType === 'dictation') { // 받아쓰기 (Step 5 로직)
                    const qItem = config.bank[getRandomInt(0, config.bank.length - 1)];
                    const p = qItem.q.replace('⬜', '<span class="text-blue-600 border-b-2 border-blue-600 inline-block min-w-[50px] text-center px-1">?</span>');
                    
                    // 오답 생성 (중복 제거)
                    let uniqueOpts = new Set([qItem.a]);
                    if (qItem.d) qItem.d.forEach(w => { if(w !== qItem.a) uniqueOpts.add(w); });
                    while(uniqueOpts.size < 4) uniqueOpts.add("오답"+uniqueOpts.size);
                ...
                else if (config.subType === 'vocab') { // 영단어 (Step 5 로직)
            ...
            if (config.type === 'skip_counting') { // Step 5 skip_counting logic inline or mapped
                 ...
                 return { problem: `<span class="text-4xl font-black text-gray-800 tracking-wider">${seq.join(' → ')}</span>`, answer: ans, options: shuffleArray(Array.from(o)) };
            }
...
if (config.type === 'word_problem') return generateWordProblem(config.mode);
            if (config.type === 'scale_interactive') return generateInteractiveScale(); // [수정됨] Code 2 로직 사용
        ...
        // ==========================================
        // [9] 게임 진행 로직 (Code 1 + Code 2 통합)
        // ==========================================

        function nextQuiz() {
            // 1. 문제 데이터 생성
            const q = generateProblem(AppState.currentSubject);
            currentQuiz = q; // 전역 변수에 저장 (정답 체크용)

            // 2. 문제 영역(HTML) 렌더링
            const contentArea = document.getElementById('quiz-content-area');
            
            // [특수 데이터 처리] 독해/달력/일과표/그림자료가 있는 경우 '보기' 버튼 생성
            let extraButtonHtml = "";
            currentModalContent = ""; // 모달 내용 초기화
            ...
            if (q.readingData) {
                ...
                extraButtonHtml = `<button onclick="showDataModal()" class="mb-4 bg-gray-400 hover:bg-gray-500 text-black font-bold py-2 px-6 rounded-full shadow-md transition animate-bounce">지문 보기</button>`;
            } 
            else if (q.monthData) {
                // 달력
                const calHtml = getCalendarHTML(q.monthData);
                currentModalContent = `<div class="p-2">${calHtml}</div>`;
                extraButtonHtml = `<button onclick="showDataModal()" class="mb-4 bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-6 rounded-full shadow-md transition">달력 보기</button>`;
            }
                ...
                extraButtonHtml = `<button onclick="showDataModal()" class="mb-4 bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-6 rounded-full shadow-md transition">일과표 보기</button>`;
            }
            else if (q.visualData) {
                // 그림 자료 (묶어 세기 등)
                currentModalContent = `<div class="p-4 flex justify-center">${q.visualData.html}</div>`;
                extraButtonHtml = `<button onclick="showDataModal()" class="mb-4 bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-6 rounded-full shadow-md transition">그림 보기</button>`;
            }
  ...
  if (q.type === 'scale_interactive') {
                    ...
                    <div class="flex flex-col items-center gap-4 w-full justify-start pt-2">
                        <div class="bg-gray-50 border-2 border-gray-200 px-2 py-1 rounded-2xl shadow-sm text-center">
                            ...
                            <div class="text-[20px] font-black text-gray-900">
                                목표: <span class="text-gray-600">${q.target}g</span>
                            </div>
                        </div>
                ...
                setTimeout(() => {
                    ...
                    optDiv.innerHTML = `
                        ...
                        <button onclick="checkAnswer(AppState.scaleState.current)" class="row-span-2 bg-blue-600 text-white font-black text-xl rounded-xl shadow-lg active:scale-95 hover:bg-blue-700">제출</button>
            ...
            // [B] 시계 UI
                ...
                // [수정 포인트] w-[240px] -> 모바일 / md:w-[280px] -> PC
                innerContent = `
                    <div class="flex flex-col items-center justify-start h-full w-full gap-4 pt-1">
                        ...
                        <div class="text-2xl md:text-3xl font-black text-gray-800 bg-gray-100 px-6 py-2 rounded-full">몇 시 몇 분?</div>
                    </div>
                `;
            ...
            // [중요] CSS에서 padding-top을 주었지만, 구조적으로 problem-content 클래스가 적용되도록 확실히 함
        ...
        function finishGameSuccess() {
            ...
            // Code 2의 랭킹 계산 로직 적용 (S컷 등)
            const rank = getRank(totalTime, TARGET_SCORE); // Step 5의 getRank 함수 사용
...
function goDashboard() {
    ...
    document.getElementById('header-title').innerText = "칠판"; 
    updateRankingBoard();
}
   // ==========================================
        // [10] 메뉴 데이터 구성 (Code 2의 모든 주제 포함)
        // ==========================================
// ==========================================
// [10] 메뉴 데이터 구성 (수학 4개 카테고리로 세분화)
// ==========================================
const MenuData = [
    {
        category: "국어", 
        items: [
            { id: "k_consonant", name: "자음 퀴즈 (ㄱ~ㅎ)" },
            { id: "k_vowel", name: "모음 퀴즈 (아~이)" },
            { id: "dict_single", name: "받아쓰기 (홑받침)" },
            { id: "dict_complex", name: "받아쓰기 (겹받침)" },
            { id: "reading_comp", name: "안내문 독해 (기초)" },
            { id: "reading_comp_hard", name: "안내문 독해 (심화)" }
        ]
    },
    {
        category: "수학: 기초 & 규칙", 
        items: [
            { id: "number_system_easy", name: "수 세기 (1~10)" },
            { id: "number_system_hard", name: "수 세기 (1~100)" },
            { id: "compare_size_easy", name: "크기 비교 (그림)" },
            { id: "compare_size_hard", name: "크기 비교 (숫자)" },
            { id: "number_bond", name: "수 모으기/가르기" },
            { id: "group_counting", name: "묶어 세기" },
            { id: "skip_counting", name: "뛰어 세기 (규칙)" },
            { id: "mul_99", name: "구구단 외우기" }
        ]
    },
    {
        category: "수학: 덧셈 & 뺄셈", 
        items: [
            { id: "add_1d", name: "1자리 덧셈" },
            { id: "sub_1d", name: "1자리 뺄셈" },
            { id: "add_2d_easy", name: "2자리 덧셈 (기초)" },
            { id: "add_2d_hard", name: "2자리 덧셈 (올림)" },
            { id: "sub_2d_easy", name: "2자리 뺄셈 (기초)" },
            { id: "sub_2d_hard", name: "2자리 뺄셈 (내림)" },
            { id: "add_3d_easy", name: "3자리 덧셈 (기초)" },
            { id: "add_3d_hard", name: "3자리 덧셈 (올림)" },
            { id: "sub_3d_easy", name: "3자리 뺄셈 (기초)" },
            { id: "sub_3d_hard", name: "3자리 뺄셈 (내림)" }
        ]
    },
    {
        category: "수학: 곱셈 & 나눗셈", 
        items: [
            { id: "mul_2d_1d", name: "2자리 × 1자리" },
            { id: "mul_2d_2d", name: "2자리 × 2자리" },
            { id: "mul_3d_2d", name: "3자리 × 2자리" },
            { id: "div_basic", name: "나눗셈 (구구단)" },
            { id: "div_hard", name: "나눗셈 (큰 수)" },
            { id: "div_rem_basic", name: "나머지 있는 나눗셈 (하)" },
            { id: "div_rem_hard", name: "나머지 있는 나눗셈 (상)" },
            { id: "div_estim", name: "몫 어림하기 (기초)" },
            { id: "div_estim_hard", name: "몫 어림하기 (심화)" }
        ]
    },
    {
        category: "수학: 분수 & 소수", 
        items: [
            { id: "fraction_basic", name: "분수 기초 (통합)" }
        ]
    },
    {
        category: "영어", 
        items: [
            { id: "life_eng", name: "생활 영어 단어" },
            { id: "alpha_order", name: "알파벳 순서" },
            { id: "alpha_case", name: "대소문자 짝꿍" },
            { id: "alpha_sound", name: "알파벳 읽기" }
        ]
    },
    {
        category: "생활", 
        items: [
            { id: "clock_quiz_easy", name: "시계 보기 (정각/30분)" },
            { id: "clock_quiz_hard", name: "시계 보기 (1분 단위)" },
            { id: "time_quiz_easy", name: "시간 계산 (기초)" },
            { id: "time_quiz_hard", name: "시간 계산 (심화)" },
            { id: "calendar_quiz", name: "달력 보기" },
            { id: "schedule_quiz", name: "일과표 분석" },
            { id: "money_quiz", name: "돈 계산 (더치페이/저축)" },
            { id: "unit_conversion", name: "단위 변환 (길이/무게/시간)" },
            { id: "word_mixed", name: "문장제 (덧셈/뺄셈)" },
            { id: "word_mul_div", name: "문장제 (곱셈/나눗셈)" },
            { id: "scale_interactive", name: "전자저울 읽기" },
            { id: "recycle_practice", name: "분리수거 왕" }
        ]
    }
];
...
function openTeacherBoard() {
    ...
    // [추가] 칠판 열 때 필기모드로 초기화
    isBoardDrawMode = false; // toggle 함수에서 반전시키므로 false로 설정 후 호출하여 true로 만듦
    toggleBoardMode(); 
    ...
    // [1] 시계 문제 (ID 변경: t-canvas -> board-sub-canvas)
    if (AppState.currentSubject.id.includes('clock') || (q.type && q.type.includes('clock'))) {
        content = `
            <div class="flex flex-col items-center justify-center w-full h-full pointer-events-auto">
                ...
                <div class="text-3xl font-black text-gray-900 drop-shadow-md">시계가 나타내는 시각은?</div>
            </div>`;
    ...
    // [2] 저울 문제 (ID 변경: t-canvas -> board-sub-canvas)
        ...
        content = `
            <div class="flex flex-col items-center justify-center w-full h-full pointer-events-auto gap-6">
                ...
                <div class="text-4xl font-black text-gray-900 drop-shadow-md">정답 무게: ${q.answer}g</div>
            </div>`;
    ...
    // 정답 보기 버튼 (우측 하단 고정)
    const ansBtn = `
        <div onclick="this.innerText='정답: ${q.answer}'; this.classList.remove('bg-gray-200', 'text-gray-500'); this.classList.add('bg-black', 'text-gray-900', 'border-white');" 
 ...
 function clearBoard() { 
            ...
            // 문제는 지우지 않고 필기만 지움. 문제를 지우려면 다시 '문제 띄우기'
        }
        ...
        function toggleBoardMode() {
            ...
            if (isBoardDrawMode) {
                ...
                btn.innerHTML = '<i class="fas fa-pen"></i> <span>필기모드</span>';
            } else {
                // 클릭 모드: 캔버스 입력 무시(뒤쪽 버튼 클릭 가능), 버튼 파란색
                ...
                btn.innerHTML = '<i class="fas fa-mouse-pointer"></i> <span>클릭모드</span>';
            }
        }
  ...
  function openTeacherStatus() {
    ...
    document.getElementById('view-teacher-status').classList.remove('hidden');
        ...
        function updateTeacherStatusBoard() {
            ...
            document.getElementById('status-table-head').innerHTML = `<th class="px-6 py-3 bg-gray-50 sticky left-0 z-10 border-r">이름</th>` + subs.map(s => `<th class="px-4 py-3 text-center text-[10px] whitespace-nowrap">${s.name}</th>`).join('');
            ...
            document.getElementById('status-table-body').innerHTML = st.map(s => {
              ...
              const myRecs = AppState.records.filter(r => String(r.userId) === String(s.id) && r.subjectId === sub.id);
                ...
                return `<tr>${r}</tr>`;
            }).join('') || `<tr><td colspan="100" class="p-8 text-center text-gray-400">데이터 없음</td></tr>`;
        }

        // ==========================================
        // [12] 개인 성적표 모달 (상세 기록)
        // ==========================================
 // [수정됨] 내 성적표 (공백 제거 및 타입 통일 적용)
function openMyInfo(targetId = null) {
    ...
    // 2. 기록 집계 (핵심 수정: 서버에서 받은 records 전체를 사용하지 않고, 랭킹 보드에서 사용한 '최고 기록' 집계 로직 재사용)
    
    // 이 로직이 서버 기록(Records 시트)에서 과목별 최고 기록 1개만 골라냅니다.
    const uniqueBest = {}; 
    let totalScore = 0;
    let sssCount = 0;
    ...
    userRecords.forEach(r => {
        // Records 시트는 이미 최고 기록만 저장하고 있으므로, 필터링만 수행합니다.
        // 하지만 혹시 모를 로컬 기록 잔존이나 서버 통신 오류에 대비하여 중복 제거 로직을 한번 더 적용
    ...
    const finalRecs = Object.values(uniqueBest).sort((a, b) => new Date(b.date) - new Date(a.date)); // 최신순 정렬
    ...
    document.getElementById('info-count').innerText = finalRecs.length; // 푼 문제 수가 아닌 '과목별 최고 기록 수'가 됨
    document.getElementById('info-table-body').innerHTML = tbody || `<tr><td colspan="4" class="px-6 py-12 text-center text-gray-400">아직 기록이 없습니다. 열심히 도전해보세요!</td></tr>`;
}

function closeMyInfo() { document.getElementById('view-my-info').classList.add('hidden'); }
   // ==========================================
        // [13] 수업 모드 (Learning Mode) 전용 로직 (Code 2 완벽 이식)
        // ==========================================

        // [13-1] 수업 모드 시작/종료
        function startLearningMode() {
            if (!AppState.currentSubject) { alert("주제를 먼저 선택해주세요."); return; }
        ...
        function exitLearningMode() {
            if(confirm("수업을 종료하고 대시보드로 돌아갈까요?")) {
                document.getElementById('view-learning').classList.add('hidden');
        ...
        function nextLearningProblem() {
            ...
            // 초기화
            clearCanvas('left'); 
            if (AppState.isScrollMode) toggleScrollMode(); // 다음 문제 넘어가면 필기 모드로 리셋
            ...
            // C. 시계/그림 등 캔버스형
                ...
                contentHtml = `
                    <div class="flex flex-col items-center justify-center h-full w-full gap-8">
                        ...
                        <div class="text-5xl font-black text-white drop-shadow-lg">몇 시 몇 분?</div>
                    </div>
                `;
                let h=0, m=0;
                if(q.h !== undefined) { h=q.h; m=q.m; }
        ...
        // [13-4] 캔버스 그리기 코어 (Code 2에서 리사이징 대응 추가)
        ...
        // ==========================================
        // [14] 앱 초기화 (Code 1 + Code 2)
        // ==========================================
    // [최종 실행] 앱 초기화 (중복 제거 및 통합)
    window.onload = () => {
        // 구글 스크립트 URL 체크
        if (GAS_URL.includes("여기에")) { 
            alert("앱스 스크립트 URL을 설정해주세요!"); 
            document.getElementById('loading-overlay').classList.add('hidden'); 
            return; 
        }
        ...
        // 2. 기능 초기화
        initClock();          // 시계
        renderSidebarMenu();  // 메뉴
        initBoard();          // 칠판
        setRole(null);        // 역할 선택 초기화
        ...
        // 메뉴 선택 시 분기 처리 (선생님 칠판 / 학생 퀴즈 / 수업 모드)
        // Code 1의 selectMenu 함수를 덮어씌워 '수업 모드' 진입점 추가 가능하도록 확장
selectMenu = function(itemJson) {
    // [수정] 메뉴 클릭 시 사이드바 닫기 (모바일 대응)
    if(sidebarOpen) toggleSidebar(); 

    if (!AppState.user) { alert("로그인이 필요합니다."); return; }
    
    const item = JSON.parse(decodeURIComponent(itemJson));
    AppState.currentSubject = item;

            // 선생님일 경우: 칠판모드 vs 수업모드 선택 (여기서는 간단하게 confirm으로 처리하거나 기본 칠판으로 감)
            // *선생님은 칠판 모드에서 '문제 띄우기'를 하거나, 별도 버튼으로 '수업 모드' 진입 가능하게 함.
            // *현재 UI상 사이드바 클릭 시 역할에 따라 자동 분기됨.
            
            // 만약 선생님이 '수업 모드(양쪽 분할 화면)'를 원한다면 
            // 칠판 화면 내에 '수업 모드로 전환' 버튼을 두거나, 
            // 여기서는 기본적으로 Code 1의 칠판 뷰로 이동합니다.
            
            if (AppState.user.role === 'teacher') {
                openTeacherBoard();
                // 칠판 뷰에 '수업 모드(필기장)' 버튼 추가 로직
...
// ==========================================
        // [MISSING LOGIC] 누락된 핵심 함수 보완
        // 이 함수들이 없으면 퀴즈가 실행되지 않습니다.
        // ==========================================
        ...
        function generateLangQuiz(type, subType, bank) {
            ...
            if (!parts) {
                ...
                options = [qItem.a, "오답1", "오답2", "오답3"];
            } else {
                const { cho, jung, jong } = parts;
                const restString = qItem.a.substring(1); // 첫 글자 뺀 나머지
                ...
                // 보기 생성 (자음퀴즈면 자음만 다르게, 모음퀴즈면 모음만 다르게)
                let o = new Set([targetChar]);
                let loop = 0;
                
                if (subType === 'consonant') { // 자음 퀴즈
            ...
            return { 
                problem: problemHtml, 
                answer: targetChar, // 정답은 한 글자 (예: '가')
                options: options 
            };
        }
        ...
        function generateComplexDiv(hasRem, level) {
            ...
            const problemHtml = `<span class="text-5xl font-black text-gray-800 tracking-wider">${dividend} ÷ ${divisor}</span>`;
                ...
                <div class="flex flex-col items-center justify-center gap-4 p-3 w-full h-full">
                    ...
                    <div class="bg-white border-2 border-yellow-400 rounded-xl p-6 shadow-lg">
                        <div class="text-4xl font-black text-gray-800 text-center leading-tight">
                            <span class="text-green-600">${divisor}</span> × <span class="text-yellow-500 animate-pulse">?</span>
                        </div>
                        <div class="mt-4 text-center text-gray-500 font-bold text-lg">
                            하면 <span class="text-black text-xl underline">${dividend}</span>와 가장 가까워요
                        </div>
                    </div>
                </div>
            `;
        ...
        // [M-5] 저울 그리기 함수 (Canvas) - Code 2에서 가져옴
        function drawInteractiveScale(id, weight) {
            const cvs = document.getElementById(id);
            if (!cvs) return;
            const ctx = cvs.getContext('2d');
        ...
        window.onload = () => {
            ...
            // 데이터 로드 및 초기화
            loadDataFromServer(); // 서버 통신
            initClock();          // 시계 작동
            renderSidebarMenu();  // 사이드바 메뉴 생성
            initBoard();          // 선생님 칠판 초기화
            setRole(null);        // 초기 역할 설정 (로그인 전)
